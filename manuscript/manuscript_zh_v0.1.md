...
**消融G：FDI牙位的形态先验作用。** 移除FDI牙位条件后，外表面Chamfer-L1增加 Δ = ... mm（95% CI：[..., ...]，p < ...），F-score@0.10 mm下降 ... 个百分点（p < ...）。FDI牙位提供特定牙齿的形态先验信息（如下颌第一磨牙的宽度-高度比、牙尖形态）和上下颌间的对应关系；其移除使模型生成的形态与目标牙位的解剖特征偏离，影响整体几何保真度。
...
该值为 ... mm（95% CI：[..., ...]），表明Structure支持误差对特征生成的平均影响约为 ... mm。相对完整模型主报告的Chamfer-L1（... mm），支持误差占总误差的比例约为 ...%，其余误差来自Feature Flow和VAE解码路径。完整结果汇总于表10；这一定量分解有助于理解两阶段框架中各模块对生成质量的分层贡献。
