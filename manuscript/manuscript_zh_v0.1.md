...
图1给出了本文提出的病例条件化牙冠生成框架。该框架以同一静态咬合坐标系下的上下颌扫描、人工标注的三维颈缘线和FDI牙位为输入，训练阶段另需技师设计的参考CAD网格作为监督，推理阶段直接生成病例适配的牙支持式后牙单冠可编辑三角网格。如图1(a)所示，多尺度牙科条件模块对输入牙列、颈缘线和FDI牙位进行对齐与采样，并分别构建Structure Flow所需的全局条件 \(G\) 与体素对齐条件 \(V\)，以及Feature Flow在预测支持 \(\hat O\) 上查询的坐标邻域条件 \(N(\hat O)\)。如图1(b)所示，定向有符号FDG表示学习模块将参考CAD网格转换为定向有符号FDG，并由稀疏几何VAE压缩为潜变量 \(Z=(O,F)\)，其中 \(O\) 为稀疏活动支持，\(F\) 为局部几何特征。如图1(c)所示，病例条件化生成模块首先由Structure Flow从噪声生成活动支持 \(\hat O\)，再由Feature Flow在其上生成局部特征 \(\hat F\)；潜变量 \(\hat Z=(\hat O,\hat F)\) 经冻结VAE解码和连续定向场重建后输出候选网格，并由独立侧支报告工程质量状态。图例中，紫色点划线表示牙科条件，蓝色实线表示生成及支持依赖，橙色实线和虚线分别表示VAE前向路径与训练监督。

上述设计以稀疏定向表示保留高分辨率形态与表面方向信息，以多尺度条件协调牙位语义、牙列空间关系和颈缘邻域约束，并通过Structure/Feature分层生成分别建模冠体整体结构和局部解剖细节。后续Sec. 3.2将介绍输入预处理与局部坐标系；Sec. 3.3介绍定向有符号FDG与稀疏几何VAE；Sec. 3.4介绍多尺度牙科条件；Sec. 3.5介绍两阶段条件流匹配；Sec. 3.6介绍训练目标；Sec. 3.7介绍连续定向场重建与网格提取。
