...
图1给出了本文提出的病例条件化牙冠生成框架。该框架包含输入预处理与局部坐标系、定向有符号FDG与稀疏几何VAE、多尺度牙科条件、两阶段条件流匹配、定向场重建与网格提取等模块，以同一静态咬合坐标系下的上下颌扫描、人工标注的三维颈缘线和FDI牙位为输入，训练阶段另以参考CAD网格为监督，推理阶段生成病例适配的牙支持式后牙单冠可编辑三角网格。如图1(a)所示，多尺度牙科条件模块对齐并采样牙列、颈缘线和FDI牙位，为Structure Flow构建全局条件 \(G\) 和体素对齐条件 \(V\)，并为Feature Flow在预测支持 \(\hat O\) 上补充坐标邻域条件 \(N(\hat O)\)。如图1(b)所示，定向有符号FDG表示学习模块将参考CAD转换为定向有符号FDG，并由稀疏几何VAE压缩为潜变量 \(Z=(O,F)\)，其中 \(O\) 为稀疏活动支持、\(F\) 为局部几何特征。如图1(c)所示，病例条件化生成模块先由Structure Flow从噪声生成活动支持 \(\hat O\)，再由Feature Flow生成局部特征 \(\hat F\)；潜变量 \(\hat Z=(\hat O,\hat F)\) 经冻结VAE解码和连续定向场重建后输出候选网格，并由独立侧支报告工程质量状态。图例中，紫色点划线表示牙科条件，蓝色实线表示生成及支持依赖，橙色实线和虚线分别表示VAE前向路径与训练监督。

上述设计以稀疏定向表示保留高分辨率形态与表面方向信息，以多尺度条件协调牙位语义、牙列空间关系和颈缘邻域约束，并通过两阶段分层生成分别建模冠体整体结构与局部解剖细节。后续将对相关模块进行具体介绍。
