...
为验证完整模型各关键设计组件对最终性能的贡献，本文通过系统消融验证五个关键设计维度：（1）定向有符号表征的有效性；（2）联合几何监督对表面与颈缘连续性的影响；（3）多尺度牙科条件的递进作用；（4）三维颈缘线、对颌牙列和FDI牙位等临床输入的必要性；（5）Structure支持预测对Feature生成的误差传播。上述五个维度分别在4.4.2.1–4.4.2.4节中展开，量化结果依次见表8、表9、补充表S3和表10；各对照的设计、训练与冻结策略和评估指标先统一汇总于表7，计算开销的横向比较见表11。Structure/Feature两阶段生成架构相对单阶段联合生成替代方案的必要性尚未纳入本轮消融，相关说明见4.4.3局限性讨论。
