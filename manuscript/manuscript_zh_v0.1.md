...
**符号说明表.** 图1中，紫色点划线表示牙科条件，蓝色实线表示生成及支持依赖路径，橙色实线与虚线分别表示VAE前向路径与训练监督。为便于图文对照，下表汇总了本文方法的关键符号与图1各元素的对应关系。

| 符号 | 含义 | 图1对应 |
|---|---|---|
| \(J_i^u,J_i^l\) | 同一静态咬合坐标系下的上、下颌扫描 | 输入牙列扫描 |
| \(M_i\) | 人工标注的有序三维颈缘线 | 颈缘线 |
| \(f_i\) | FDI牙位 | FDI牙位 |
| \(S_i\) | 参考CAD蜡型（wax-up）网格 | 参考CAD |
| \(\hat S_i\) | 生成的候选冠体网格 | 候选冠体网格 |
| \(Z=(O,F)\) | 稀疏潜变量 | VAE潜变量 |
| \(O\) | \(64^3\) 潜空间中的活动支持 | 活动支持 |
| \(F\) | 活动坐标上的局部几何特征 | 局部特征 |
| \(C^g\) | 全局牙科条件 | G |
| \(C^v\) | 体素对齐牙科条件 | V |
| \(C^n\) | 坐标邻域牙科条件 | \(N(\hat O)\) |
| \(C^S=\{C^g,C^v\}\) | Structure阶段牙科条件 | G、V |
| \(C^F=\{C^g,C^v,C^n\}\) | Feature阶段牙科条件 | G、V、\(N(\hat O)\) |
| \(\hat O\) | Structure Flow预测的活动支持 | 预测支持 \(\hat O\) |
| \(\hat F\) | Feature Flow预测的局部特征 | 局部特征 \(\hat F\) |
| \(\hat Z=(\hat O,\hat F)\) | 预测的稀疏潜变量 | 潜变量 \(\hat Z\) |

![图1. 病例条件化牙冠生成框架。（a）上下颌扫描、三维颈缘线和FDI牙位经对齐与采样后，由两个参数独立的编码器形成Structure阶段的全局与体素对齐条件（G、V），以及Feature阶段在预测支持 \(\hat O\) 上查询的坐标邻域条件（G、V、\(N(\hat O)\)）。（b）参考CAD被转换为定向有符号FDG，并通过可训练的稀疏VAE学习潜变量 \(Z=(O,F)\)；目标与解码FDG形成重建监督，潜变量后验由KL项正则化，重建网格仅用于自由解码展示与评价。（c）Structure Flow由噪声生成结构潜变量，冻结的结构解码器将其恢复为预测活动支持 \(\hat O\)；Feature Flow以 \(\hat O\) 及定义在其上的噪声为输入生成局部特征 \(\hat F\)。二者组成的潜变量 \(\hat Z=(\hat O,\hat F)\) 经冻结VAE解码和连续定向场重建得到候选冠体网格，并由独立侧支报告工程质量状态。紫色点划线表示牙科条件，蓝色实线表示生成及支持依赖路径，橙色实线与虚线分别表示VAE前向路径与训练监督。](../figures/method_framework.png)
...
本研究使用的回顾性数据集来自国内数据提供机构及多个技工厂的去标识化口内扫描与技师设计CAD蜡型。最终预处理纳入24,049例有效病例，病例来源...（待填写）。每例同时包含上下颌扫描、清晰三维颈缘线、至少4颗邻牙及正确咬合关系。排除标准包括颈缘点到参考网格平均最近距离大于0.5 mm、参考网格中超出24 mm局部立方体比例大于2%、FDG活动单元数超过3,000,000等预设质量要求。
