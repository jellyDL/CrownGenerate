...
图1给出了病例条件化牙冠生成框架，包含输入预处理与局部坐标系、定向有符号FDG表示学习、多尺度牙科条件、两阶段条件流匹配、定向场重建与网格提取五个模块。该框架以上下颌扫描、三维颈缘线和FDI牙位为输入，以参考CAD网格为监督，生成牙支持式后牙单冠网格。图1(a)中，该模块采样牙列、颈缘线和FDI牙位，为Structure Flow构建 \(G\) 和 \(V\)，为Feature Flow补充 \(N(\hat O)\)。图1(b)中，FDG表示学习模块将参考CAD压缩为 \(Z=(O,F)\)。图1(c)中，Structure Flow生成活动支持 \(\hat O\)，Feature Flow生成局部特征 \(\hat F\)；\(\hat Z=(\hat O,\hat F)\) 经VAE解码和定向场重建输出候选网格。
...
图1. 病例条件化牙冠生成框架。（a）对齐并采样上下颌扫描、三维颈缘线和FDI牙位；两个参数互不共享的条件编码器分别构建Structure阶段的全局与体素对齐条件（G、V），以及Feature阶段在 \(\hat O\) 上查询的坐标邻域条件（G、V、\(N(\hat O)\)）。（b）将参考CAD转换为定向有符号FDG，并由稀疏VAE编码为 \(Z=(O,F)\)；目标与解码FDG提供重建监督，KL项正则化潜变量后验。（c）Structure Flow由噪声生成 \(\hat O\)，Feature Flow在 \(\hat O\) 上生成 \(\hat F\)；\(\hat Z=(\hat O,\hat F)\) 经冻结VAE解码和连续定向场重建为候选网格，独立侧支报告工程质量状态。紫色点划线表示牙科条件，蓝色实线表示生成及支持依赖，橙色实线和虚线分别表示VAE前向路径与训练监督。
...
该环节对应图1(a)中两个参数独立的牙科条件编码器，分别向Structure阶段提供全局/体素对齐条件（G、V），向Feature阶段提供全局/体素对齐/坐标邻域条件（G、V、\(N(\hat O)\)）。
...
图3进一步展开Feature Flow的坐标级条件融合。对活动坐标 \(q\in\hat O\)，\(C^g\) 与体素条件 \(C^v\) 构成交叉注意力标记，\(C^g\) 与其在 \(q\) 处的读出 \(C^v(q)\) 则以残差方式注入潜变量；\(C^n(q)\) 与预测支持的稀疏轴向上下文 \(A(q;\hat O)\) 共同调节局部特征。后者的具体构造见Sec. 3.5.3。

![图3. Feature Flow的坐标级多尺度条件融合](../figures/feature_condition_detail.png)
图3. Feature Flow的坐标级多尺度条件融合（a）病例对齐后，以预测活动支持 \(\hat O\) 中的坐标 \(q\) 为查询位置；上、下颌、三维颈缘线和FDI牙位共同定义其病例环境。（b）全局条件 \(C^g\) 与体素条件 \(C^v\) 构成交叉注意力标记；\(C^g\) 及其在 \(q\) 处的体素读出 \(C^v(q)\) 以残差方式作用于Feature token。上、下颌和颈缘近邻构成局部牙科描述 \(C^n(q)\)，预测支持中的多尺度轴向邻域构成 \(A(q;\hat O)\)。这些信息在稀疏Feature DiT中融合以预测局部几何特征 \(\hat F_q\)。紫色点划线表示牙科条件路径，蓝色实线表示活动支持与特征路径。


### 3.5 两阶段条件流匹配

该环节对应图1(c)中Structure Flow与Feature Flow的生成过程：Structure Flow由噪声生成结构潜变量并恢复为预测活动支持 \(\hat O\)；Feature Flow在 \(\hat O\) 上生成局部特征 \(\hat F\)；二者组成 \(\hat Z=(\hat O,\hat F)\) 后解码重建候选网格。
...
Structure Flow首先确定冠体潜变量的空间支持。VAE潜变量活动坐标被转换为 \(64^3\) 二值占位 \(O\)，再由冻结的TRELLIS结构编码器压缩为 \(16^3\)、8通道结构潜变量 \(x_0^S\)[8,9]。以全局和体素对齐条件 \(C^S\) 为输入，三维DiT预测结构潜空间中的条件速度场，并由噪声生成 \(x_0^S\)。

推理时，冻结的结构解码器将生成的 \(x_0^S\) 还原为 \(64^3\) 占位分数，并据此确定Feature Flow的活动坐标。

#### 3.5.3 Feature Flow

给定活动支持后，Feature Flow在各活动坐标上生成标准化的局部几何潜变量 \(x_0^F\)。Feature训练与单阶段潜变量验证使用参考CAD经VAE编码得到的活动坐标，使局部特征学习不受Structure误差干扰；端到端生成可视化、网格评价与正式推理则使用Structure预测坐标。对每个活动坐标 \(q\)，稀疏Feature DiT联合输入噪声潜变量、全局条件 \(C^g\)、体素读出 \(C^v(q)\)、坐标邻域条件 \(C^n(q)\) 以及活动支持上下文 \(A(q;O)\)，并预测局部条件速度。膨胀率为1、2和4的18个轴向邻居用于编码多尺度活动支持，潜在特征之间的交互由稀疏DiT完成。
...
该环节对应图1(c)中由 \(\hat Z=(\hat O,\hat F)\) 经冻结VAE解码、连续定向场重建与网格提取得到最终候选网格的右半部分。
...
该构造同时利用局部距离和方向，旨在缓解仅插值离散距离造成的表面断裂。连续场经高斯平滑、负场孔填充、最大体素连通域保留及裁剪框边界清除后，以Marching Cubes提取零水平集[22]；所得网格再经最大面连通分量保留、顶点焊接、退化面删除、有限小孔修复和绕向统一。网格清理与工程质量门禁按表3执行。
...
本研究使用的回顾性数据集来自国内数据提供机构及多个技工厂的去标识化口内扫描与技师设计CAD蜡型。最终预处理纳入24,049例有效病例。每例同时包含上下颌扫描、清晰三维颈缘线、至少4颗邻牙及正确咬合关系。排除标准包括颈缘点到参考网格平均最近距离大于0.5 mm、参考网格中超出24 mm局部立方体比例大于2%、FDG活动单元数超过3,000,000等预设质量要求。

所有病例均已移除姓名、年龄、性别、机构和日期等身份信息。预处理流程包括牙科局部坐标系建立、点云采样、颈缘线重采样及FDG构建，详见Sec. 3.2。

#### 4.1.2 数据集划分

训练、验证和测试集按病例以85%/10%/5%固定划分，分别为20,441、2,405和1,203例。采用分层随机抽样保持FDI牙位、上下颌及左右分布的一致性。当前划分为病例级随机划分，尚未进行患者级或扫描级去重；同一患者或同一上下颌扫描可能因多颗修复牙形成多个病例并跨越数据集，潜在数据相关性在讨论中作为限制说明。测试集限定为后牙单冠，不含前牙。
