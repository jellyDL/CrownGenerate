...
![图1. 病例条件化牙冠生成框架。顶部给出训练监督：参考CAD经定向有符号FDG和稀疏VAE编码为潜变量 \(Z=(O,F)\)；其中，\(O\) 经冻结结构编码器形成结构目标 \(x^S\)，\(F\) 构成特征目标 \(x^F\)，参考潜变量的解码场提供表面目标 \((d,n)\)，并分别形成 \(L_S\)、\(L_F\) 和 \(L_{\mathrm{surf}}\)。中部为生成主链：Structure Flow生成 \(x^S\)，冻结的结构解码器将其恢复为活动支持 \(O\)，Feature Flow在该支持上预测局部特征 \(F\)；二者经冻结的VAE解码器与连续定向场重建，输出候选冠体网格及工程质量状态。底部为牙科条件：上下颌扫描、三维颈缘线和FDI牙位经对齐与采样后，形成全局（G）、体素对齐（V）和坐标邻域（N）条件，并分别注入两个Flow阶段。蓝色实线、紫色点划线和橙色虚线分别表示生成、牙科条件和训练监督路径。](../figures/method_framework.png)
