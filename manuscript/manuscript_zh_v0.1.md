...
**性能归因。** 本文方法的预期性能来源可归结为三方面设计的协同：定向有符号FDG在稀疏活动单元上联合编码局部几何、截断有符号距离与定向法向，使生成过程能够区分表面两侧并保持朝向一致；Structure/Feature两阶段条件流匹配将冠体活动支持与局部细节分层建模，避免单一尺度同时承担整体结构与高频解剖细节的生成；全局、体素对齐和坐标邻域三级条件在不同空间尺度注入牙位语义、牙列空间对应和颈缘邻域约束，使颈缘连续性与静态咬合协调在生成阶段即被显式建模。上述机制性归因的定量证据以表11–表13的消融结果为准；在独立重训消融与多种子稳定性结果冻结前，不对具体模块作因果性表述。
