...
上述输出分别提供病例级、体素级和坐标级牙科信息。图3概述其编码过程；具体的阶段分配与坐标级注入见第3.5节。

![图3. 多尺度牙科条件](../figures/dental_condition_encoder.png)
图3. 多尺度牙科条件概览。三张编号卡片分别对应第3.4节定义的全局、体素对齐和坐标邻域条件。全局条件 \(C_d^g\) 由三类来源特征的均值/最大池化与FDI嵌入构成512维病例级表示；体素对齐条件由 \(64^3\) 的 \(C_{d,64}^v\) 下采样为 \(16^3\) 的 \(C_{d,16}^v\)；坐标邻域条件 \(C_F^n(q)\) 仅用于Feature阶段，由查询坐标及每类来源的4个近邻构成的 \(3\times4\times7\) 描述经多层感知机投影得到。\(d\in\{S,F\}\) 的两个条件编码器参数不共享。
...
![图4. Feature Flow的坐标级多尺度条件融合](../figures/feature_condition_detail.png)
图4. Feature Flow的坐标级多尺度条件融合（推理阶段）。（a）病例在局部坐标系中对齐后，以预测活动支持 \(\hat O\) 中的坐标 \(q\) 为查询位置；上、下颌、三维颈缘线和FDI牙位共同定义该位置的病例上下文。（b）\(C_F^g\) 和 \(C_{F,16}^v\) 构成交叉注意力标记；\(C_F^g\) 及其在 \(q\) 处从 \(64^3\) 场读取的体素读出 \(C_{F,64}^v(q)\) 经残差路径注入Feature token；\(C_F^n(q)\) 与由预测支持构建的稀疏轴向上下文 \(A(q;\hat O)\) 一并作用于该token。稀疏Feature DiT融合这些输入以预测局部几何特征 \(\hat F_q\)。紫色点划线表示牙科条件路径，蓝色实线表示活动支持与特征路径。
