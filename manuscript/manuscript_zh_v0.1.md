...
整个生成框架如图 1 所示，包含四个环节：：(i) 输入预处理与牙科局部坐标系建立；(ii) 定向有符号柔性双网格表示与稀疏几何VAE压缩；(iii) 基于多尺度的牙科条件；(iv)两阶段条件流匹配 (v)连续定向距离场重建与网格提取.以下章节将详细介绍这些模块。

对病例 \(i\)，输入 \(C_i=(J_i^u,J_i^l,M_i,f_i)\) 包括同一静态咬合坐标系下的上下颌扫描、人工标注的有序三维颈缘线和FDI牙位。训练时另提供技师设计的参考CAD蜡型网格 \(S_i\)，推理时由 \(C_i\) 生成病例适配的候选网格 \(\hat S_i\)。框架依次完成病例对齐、稀疏几何编码、多尺度条件构建、两阶段生成及连续定向场网格重建。
...
其中，Structure Flow根据条件 \(C_i^S\) 生成活动支持 \(O_i\)，Feature Flow在 \(O_i\) 上结合条件 \(C_i^F\) 预测局部特征 \(F_i\)，再经VAE解码和连续定向场重建得到可编辑网格。输出仅作为牙支持式后牙单冠外部形态的CAD初稿，不包含临床冠内表面、粘固间隙、制造补偿或加工参数。

![图1. 病例条件化牙冠生成框架。](../figures/method_framework.png)
图1. 病例条件化牙冠生成框架。（a）对齐并采样上下颌扫描、三维颈缘线和FDI牙位；两个参数独立的编码器分别构建Structure阶段的全局与体素对齐条件（G、V），以及Feature阶段在 \(\hat O\) 上查询的坐标邻域条件（G、V、\(N(\hat O)\)）。（b）将参考CAD转换为定向有符号FDG，并由稀疏VAE编码为 \(Z=(O,F)\)；目标与解码FDG提供重建监督，KL项正则化潜变量后验。（c）Structure Flow由噪声生成 \(\hat O\)，Feature Flow在 \(\hat O\) 上生成 \(\hat F\)；\(\hat Z=(\hat O,\hat F)\) 经冻结VAE解码和连续定向场重建为候选网格，独立侧支报告工程质量状态。紫色点划线表示牙科条件，蓝色实线表示生成及支持依赖，橙色实线和虚线分别表示VAE前向路径与训练监督。
