...
\hspace{2em}Designing the external morphology of tooth-supported posterior single crowns is a key three-dimensional generation task in digital restorative dentistry. The crown must reproduce fine anatomical features, including cusps, ridges, grooves, and fossae, while maintaining a continuous contour along the manually delineated three-dimensional margin line and appropriate spatial relationships with adjacent teeth and the opposing dentition under static occlusion. Existing computer-aided design and manufacturing (CAD/CAM) systems typically require dental technicians to select a standard library tooth and iteratively position, deform, and locally sculpt it according to intraoral scans, making the process time-consuming and operator-dependent [1,2,33]. Accordingly, this study addresses the generation of a case-adapted, editable crown mesh conditioned on registered full-arch maxillary and mandibular scans, a three-dimensional margin line, and the FDI tooth designation. The scope is limited to the external morphology of tooth-supported posterior single crowns and excludes intaglio-surface design, cement-space definition, manufacturing compensation, and fabrication-ready restoration generation.

\hspace{2em}Data-driven dental crown design has progressed from two-dimensional occlusal-surface reconstruction to three-dimensional generation using point clouds, meshes, voxels, and implicit fields. DentalRecNet recovers missing surfaces from occlusal-view depth projections; however, its single-view representation cannot fully capture axial surfaces, cervical margins, or occluded regions [3]. Point-to-mesh methods and the DCrownFormer series predict crown points and normals, followed by differentiable Poisson reconstruction or implicit mesh refinement to obtain continuous surfaces [4,5,34]. VBCD adopts a voxel-driven coarse-to-fine strategy with tooth-position prompts and margin-related supervision [35]. MADCrowner incorporates margin awareness into template deformation, refinement, and post-processing to reduce boundary overextension during surface reconstruction [33]. ToothCraft performs case-conditioned tooth completion through conditional diffusion over signed distance fields [6]. Although these methods improve case adaptation, boundary control, and surface detail, representation-induced information loss and computational overhead still hinder the simultaneous preservation of fine anatomy, marginal continuity, and global surface consistency.

\hspace{2em}Recent advances in general-purpose three-dimensional generative models have provided new representational and generative foundations for addressing these challenges. Flow Matching enables continuous generation by learning a velocity field along a probability path [7]. TRELLIS and TRELLIS.2 hierarchically model sparse occupancy structures and local geometric features using structured latent representations, thereby supporting high-resolution three-dimensional generation [8,9]. SparseFlex and Direct3D-S2 further exploit surface-neighborhood sparsity to alleviate the storage and computational costs of dense voxels, which grow cubically with resolution [10,12]. However, these methods are designed primarily for general three-dimensional assets conditioned on images or text; their conditioning mechanisms and optimization objectives do not explicitly encode tooth-position semantics, maxillomandibular relationships, or three-dimensional margin-line geometry. Consequently, directly adapting these models to dental crown design makes it difficult to jointly preserve global crown structure, high-frequency occlusal morphology, and case-specific spatial constraints.

\hspace{2em}To address these limitations, we propose a case-conditioned three-dimensional dental crown generation framework based on an oriented signed Flexible Dual Grid (FDG) and multiscale conditional flow matching. The oriented signed FDG instantiates active cells only within the crown surface neighborhood and jointly encodes geometry, truncated signed distances, and oriented surface normals. A sparse variational autoencoder compresses this representation into a compact latent space, preserving high-resolution shape and directional information while controlling computational cost. Structure Flow first generates the sparse crown occupancy, after which Feature Flow predicts local geometric features, enabling hierarchical modeling of global structure and anatomical detail. Global, voxel-aligned, and coordinate-neighborhood conditions encode tooth-position semantics, spatial correspondences, and local constraints imposed by the margin and opposing dentition, respectively, thereby enabling cross-scale fusion of heterogeneous dental information. Latent-space and decoded-surface supervision further constrain local geometry and surface consistency, reducing discontinuities and local artifacts near the margin. Together, these components balance computational efficiency, anatomical fidelity, and case-specific spatial compatibility while producing an editable triangle mesh.
