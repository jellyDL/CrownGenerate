...
**比较方案与主要结果。** 在统一测试协议下，本文比较完整模型、FDI牙位最近邻基线、MADCrowner[33]、VBCD[35]和DCrownFormer+[34]，并将DCrownFormer+预设为主要外部比较器。表6同时报告运行覆盖和关键几何终点：成功生成、终点可评价和质量门禁通过均以完整测试集\(N\)为分母；几何比较在共同终点可评价的\(n=...\)例病例中进行。未生成、不可评价或未通过门禁的病例保留在运行覆盖统计中，不以门禁通过子集替代比较分母。完整模型与DCrownFormer+的外表面Chamfer-L1分别为...和... mm，病例内配对差值为... mm（95% CI：...）。

**表6. 各方法的运行覆盖与主要几何结果**

| 方法 | 成功生成，\(n/N\)（%） | 终点可评价，\(n/N\)（%） | 质量门禁通过，\(n/N\)（%） | Chamfer-L1（mm）↓ | HD95（mm）↓ | F-score@0.10 mm↑ | 颈缘线P95误差（mm）↓ |
|---|---|---|---|---|---|---|---|
| FDI最近邻 | ... | ... | ... | ... | ... | ... | ... |
| MADCrowner | ... | ... | ... | ... | ... | ... | ... |
| VBCD | ... | ... | ... | ... | ... | ... | ... |
...
注：运行覆盖列以完整测试集为分母；几何列在共同终点可评价病例集上计算。其余预设的距离、法向和曲率指标见补充材料。

**定性形态比较。** 图4以预先确定的同一病例、固定视角和参考CAD定义的局部区域展示整体冠体、咬合细节和颈缘。各方法应使用相同的渲染尺度和颜色设置；该图仅用于辅助解释表6的几何结果，不构成独立终点或方法排序。

![图4. 同一病例的定性比较版式预览。列依次为参考CAD、FDI最近邻、MADCrowner、VBCD、DCrownFormer+和本文完整模型；行依次为整体冠体、咬合细节和颈缘细节。当前图为非结果版式预览，须以冻结运行网格替换。](../figures/figure4_method_comparison_illustrative_preview.png)

**静态虚拟对颌与拓扑结果。** 表7报告关键接触/近接触分布、穿透和网格质量。接触/近接触面积不作单向优劣解释，而与参考CAD分布、穿透指标和拓扑完整性共同判断。完整模型与DCrownFormer+的穿透面积差值为...个百分点（95% CI：...），严重穿透病例率差值为...个百分点（95% CI：...）。

**表7. 各方法的关键静态虚拟对颌与拓扑结果**

| 方法 | 接触/近接触面积（%） | 穿透面积（%）↓ | 穿透深度HD95（mm）↓ | 严重穿透病例，\(n/N\)（%）↓ | 水密网格，\(n/N\)（%）↑ | 单连通genus-0网格，\(n/N\)（%）↑ |
|---|---|---|---|---|---|---|
| 参考CAD（非比较方法） | ... | ... | ... | ... | ... | ... |
| FDI最近邻 | ... | ... | ... | ... | ... | ... |
| MADCrowner | ... | ... | ... | ... | ... | ... |
...
注：接触/近接触面积、穿透面积和穿透深度在共同终点可评价病例集上计算；拓扑列以各方法成功生成网格为分母，参考CAD以其可评价病例为分母。参考CAD仅提供静态对颌分布的解释性锚点，不参与方法排序；其余预设拓扑指标见补充材料。

**技师评价与工作流时间。** 图5按病例级共识呈现来源盲法下人工路径与本文完整模型的修改需求分布；同一病例的多名技师评分在汇总前达成共识，不作为独立观测。评分者间一致性、来源猜测正确率和病例内工作流时间差将在冻结后以效应量及95% CI报告。
