...
稀疏几何VAE、Structure Flow和Feature Flow依次训练。对应目标分别为VAE重建与几何一致性、Structure流匹配，以及Feature潜变量、解码表面和短轨迹终点监督；训练策略见第4.3节。
...
在接近数据端的时间区间内，冻结VAE在不使用细分教师强制（teacher forcing）的条件下计算\(\mathcal L_{\mathrm{surf}}\)，约束重叠FDG支持上的几何、符号、方向及邻域一致性。\(\mathcal L_{\mathrm{end}}\)通过短程条件Euler轨迹约束终点潜变量误差。训练参数见第4.3节。

### 3.7 连续定向场重建与网格提取

如图1(c)所示，预测潜变量\(\hat Z=(\hat O,\hat F)\)经冻结VAE解码、连续定向场重建和网格提取后，得到最终候选网格。

推理时，每例仅生成一个候选。Structure Flow和Feature Flow从高斯噪声反向积分至数据端，并采用条件引导；前者生成并门控活动支持，后者在该支持上生成局部几何潜变量，再由VAE解码为稀疏FDG几何、距离和法向。具体采样、支持约束和随机性设置见第4.3节。
...
该场联合局部距离与方向信息以缓解表面断裂。经平滑、连通域筛选和边界清理后，使用Marching Cubes提取零水平集[22]，并完成网格清理与质量门禁；具体设置见第4.3节。
...
在相同病例划分和评价流程下，本文比较完整模型、FDI牙位最近邻基线、MADCrowner[33]、VBCD[35]和DCrownFormer+[34]，其中DCrownFormer+作为主要外部比较器。各方法完成其原生生成和必要的表面重建后，均采用第4.2节定义的外表面切割、距离计算和质量检查流程评估。表6报告完整测试集上的生成覆盖，以及全部方法共同终点可评价病例（\(n=...\)）上的主要几何结果。

**表6. 各方法的生成覆盖与主要几何结果**

| 方法 | 生成完成，\(n/N\)（%） | Chamfer-L1（mm）↓ | HD95（mm）↓ | F-score@0.10 mm↑ | 颈缘线P95误差（mm）↓ |
|---|---|---|---|---|---|
| FDI最近邻 | ... | ... | ... | ... | ... |
| MADCrowner | ... | ... | ... | ... | ... |
| VBCD | ... | ... | ... | ... | ... |
...
注：生成完成指推理链路输出可解析的候选网格，独立于后续质量门禁；生成完成率以完整测试集为分母。几何列在全部方法共同终点可评价的病例集上计算。其余预设距离、法向和曲率指标见补充材料。

如表6所示，完整模型的Chamfer-L1、HD95、F-score@0.10 mm和颈缘线P95误差分别为...、...、...和...；DCrownFormer+的相应结果为...、...、...和...。两者的Chamfer-L1病例内配对差值为... mm，完整模型的生成完成率为...（.../...）。

除表面几何保真度外，本文进一步在最终网格上评估静态虚拟对颌关系和拓扑完整性。接触/近接触面积须与参考CAD分布、穿透程度及网格完整性联合解释，而不作为单一方向的比较指标；相应结果汇总于表7。

**表7. 各方法最终网格的静态虚拟对颌与拓扑结果**

| 方法 | 接触/近接触面积（%） | 穿透面积（%）↓ | 穿透深度HD95（mm）↓ | 严重穿透病例，\(n/n_{\mathrm{eval}}\)（%）↓ | 水密且绕向一致，\(n/N_{\mathrm{row}}\)（%）↑ | 单连通genus-0网格，\(n/N_{\mathrm{row}}\)（%）↑ |
|---|---|---|---|---|---|---|
| 参考CAD（非比较方法） | ... | ... | ... | ... | ... | ... |
| FDI最近邻 | ... | ... | ... | ... | ... | ... |
| MADCrowner | ... | ... | ... | ... | ... | ... |
...
注：\(n_{\mathrm{eval}}\)为共同终点可评价病例数。对于预测方法，\(N_{\mathrm{row}}\)为生成完成的网格数；对于参考CAD，\(N_{\mathrm{row}}\)为其可评价病例数。参考CAD仅提供静态对颌分布的解释性锚点，不参与方法排序。其余预设拓扑指标见补充材料。

表7中，完整模型的接触/近接触面积、穿透面积和穿透深度HD95分别为...、...和...；严重穿透病例为.../...，水密且绕向一致网格及单连通genus-0网格分别为.../...和.../...。DCrownFormer+的相应结果为...、...、...、.../...、.../...和.../...。

冻结结果图将展示同一预先确定病例中各方法生成的整体冠体、咬合沟窝和颈缘区域。各模型应采用相同的视角、渲染尺度和表面距离色标，误差图表示相对于参考CAD的表面距离，用于辅助解释表6和表7的量化结果。

![图4. 同一病例定性比较的预览版式。冻结结果图应按列展示参考CAD、FDI最近邻、MADCrowner、VBCD、DCrownFormer+和本文完整模型，并按行展示整体冠体、咬合细节和颈缘细节。当前模板不对应本文任一方法或结果，须以真实网格及统一色标的表面距离误差图替换。](../figures/figure4_method_comparison_illustrative_preview.png)

盲评和运行时间结果冻结后，图5将报告来源盲法下人工路径和本文完整模型的病例级修改需求。每例由多名技师评分并达成共识，故以病例为分析单位。人工路径与本文完整模型在无需、轻度、中度和重度修改等级上的比例分别为.../.../.../...和.../.../.../...；评分者间一致性、来源猜测正确率和病例内工作流时间差分别为...、...和...。

![图5. 来源盲法技师评价的病例级修改需求分布。左、右环形图分别表示人工路径和本文完整模型；扇区为预先定义的无需、轻度、中度和重度修改病例比例，中心为有效病例数。当前图为非结果版式模板，须以冻结盲法评分替换；模板扇区不代表比例。](../figures/figure5_technician_score_distribution_template.png)
...
**统一协议。** 所有消融采用相同的数据划分、预处理、单候选推理和质量门禁；几何与静态虚拟对颌指标均在共同终点可评价病例集上进行病例内配对比较。除Structure误差传播诊断外，每个对照仅改变一个预设因素，未生成或不可评价病例仍按运行覆盖单独报告。完整配置记为\(F_{\mathrm{full}}=\mathrm{A2}=\mathrm{C2}=\mathrm{P1}\)，即同一冻结运行的定向表征、联合监督、全部病例条件和预测Structure支持。

**变体训练与冻结。** A0--A2作为表征/监督变体，分别重训相应VAE和生成链路；C0--C2及I-Margin、I-Opposing和I-FDI固定A2 VAE后，分别重训Structure/Feature Flow及条件适配器，并在训练和推理的全部尺度移除相应条件。P0/P1不重训，使用同一\(F_{\mathrm{full}}\)检查点、同一病例级随机性和推理设置，仅替换Feature阶段的支持来源。表8列出各独立对照及其主要终点。

**表8. 独立消融对照定义**

| 消融 | 对照配置 | 唯一改变项 | 执行方式 | 主要终点 |
|---|---|---|---|---|
| A | A0 vs. A1 | 标准FDG替换为截断有符号距离和定向法向 | 表征变体独立重训 | Chamfer-L1、HD95、法向一致性 |
| B | A1 vs. A2 | 加入梯度、Laplacian、线积分一致性和细节监督 | 表征变体独立重训 | 颈缘线P95误差、局部法向变化 |
| C | C0 vs. C1 | 在全局条件上加入体素对齐条件 | 固定A2 VAE，独立重训Flow/适配器 | Chamfer-L1、HD95 |
| D | C1 vs. C2 | 在全局和体素条件上加入坐标邻域条件 | 固定A2 VAE，独立重训Flow/适配器 | 颈缘线P95误差、穿透面积 |
| E | 完整配置 vs. I-Margin | 去除三维颈缘线条件 | 所有尺度移除条件，独立重训 | 颈缘线P95误差 |
| F | 完整配置 vs. I-Opposing | 去除对颌牙列条件 | 所有尺度移除条件，独立重训 | 穿透面积、穿透深度HD95 |
| G | 完整配置 vs. I-FDI | 去除FDI牙位条件 | 所有尺度移除条件，独立重训 | Chamfer-L1、F-score@0.10 mm |
| H | P0 vs. P1 | Feature阶段使用参考导出支持或预测支持 | 同一冻结\(F_{\mathrm{full}}\)诊断 | \(\delta_{\mathrm{Structure}}\) |

##### 4.4.2.1 定向有符号表征与表面监督

**消融A：定向有符号表征。** A0与A1保持病例条件、Structure协议和基础重建目标一致，仅将标准FDG替换为截断有符号距离和定向法向。两者的外表面Chamfer-L1分别为...和... mm，病例内配对差值为... mm（95% CI：...）；HD95和有向法向一致性见表9。该对照估计有符号距离和定向法向作为联合表征包的作用。

**消融B：表面几何监督。** A1与A2均使用定向有符号表征，A2仅额外加入梯度、Laplacian、线积分一致性和细节监督。A2相对A1的颈缘线P95误差差值为... mm（95% CI：...），局部法向变化差值为...。该对照反映联合监督包的贡献，不将其拆解为单个损失项的因果效应。

**表9. 定向表征（A0 vs. A1）与表面监督（A1 vs. A2）的独立消融结果**

| 指标 | A0：标准FDG | A1：+有符号距离/定向法向 | A2：+联合几何监督 |
|---|---|---|---|
| 外表面Chamfer-L1（mm） | ... | ... | ... |
| 外表面HD95（mm） | ... | ... | ... |
| 有向法向一致性 | ... | ... | ... |
...
注：A0--A1和A1--A2是两个独立的嵌套对照；所有指标在相同的共同终点可评价病例集上计算。A2、C2和P1均指\(F_{\mathrm{full}}\)，必须复用同一冻结运行；如需估计跨随机种子稳定性，应另行报告重复训练次数和病例级汇总结果。

##### 4.4.2.2 多尺度条件注入

**消融C：体素对齐条件。** C0与C1均采用完整表征和监督，并保留全局条件；C1仅增加体素对齐条件，坐标邻域条件保持关闭。C1相对C0的外表面Chamfer-L1配对差值为... mm（95% CI：...），HD95差值为... mm。

**消融D：坐标邻域条件。** C1与C2均保留全局和体素对齐条件，C2仅增加坐标邻域查询。C2相对C1的颈缘线P95误差差值为... mm（95% CI：...），静态虚拟对颌穿透面积差值为...个百分点。该对照只检验局部坐标邻域条件的增量。

**表10. 体素对齐（C0 vs. C1）与坐标邻域（C1 vs. C2）的独立条件消融结果**

| 指标 | C0：仅全局 | C1：全局+体素 | C2：全局+体素+坐标邻域 |
|---|---|---|---|
| 外表面Chamfer-L1（mm） | ... | ... | ... |
| 外表面HD95（mm） | ... | ... | ... |
| 颈缘线邻近误差（mm） | ... | ... | ... |
...
注：C0--C1和C1--C2是两个独立的嵌套对照；所有行采用与表9相同的测试病例和评估协议。

##### 4.4.2.3 输入条件必要性

**消融E：三维颈缘线。** I-Margin仅在训练和推理的全部尺度移除三维颈缘线，其余完整配置保持不变。相对完整配置，颈缘线P95误差的配对差值为... mm（95% CI：...）。

**消融F：对颌牙列。** I-Opposing仅在训练和推理的全部尺度移除对颌牙列条件，其余完整配置保持不变。相对完整配置，穿透面积差值为...个百分点（95% CI：...），穿透深度HD95差值为... mm。

**消融G：FDI牙位。** I-FDI仅在训练和推理的全部尺度移除FDI牙位条件，其余完整配置保持不变。相对完整配置，外表面Chamfer-L1差值为... mm（95% CI：...），F-score@0.10 mm差值为...。三项输入消融的完整指标、运行覆盖和质量门禁结果见补充表S1。

##### 4.4.2.4 Structure支持误差传播

**消融H：支持来源。** 表11比较Feature阶段采用参考CAD导出支持（P0，oracle-support）与预测支持（P1，end-to-end）的结果。P0仅作为支持误差传播的诊断条件，而非端到端方法比较器；P1相对P0的外表面Chamfer-L1增量定义为\(\delta_{\mathrm{Structure}}\)，其值为... mm（95% CI：...）。

**表11. Structure支持来源（P0 vs. P1）的误差传播结果**

| 指标 | P0：参考导出支持 | P1：预测支持 | 差值（95% CI） |
|---|---|---|---|
| 外表面Chamfer-L1（mm） | ... | ... | ... |
| 外表面HD95（mm） | ... | ... | ... |
| 有向法向一致性 | ... | ... | ... |
...
**计算开销。** 训练时间、单例推理时间和峰值显存应与消融结果同时报告；完整配置相对相应对照的差值分别为...、... ms和... MB。该比较用于描述计算代价，不用于推断临床收益。

#### 4.4.3 讨论

**结果解释。** 完整模型的表现应由共同可评价病例中的主要几何终点、运行完整性和静态对颌/拓扑指标共同解释（表6–7），不能以单一案例或各自门禁通过子集替代总体结论。定向有符号FDG、Structure/Feature分层生成和多尺度牙科条件分别面向局部几何编码、整体支持预测以及颈缘与对颌约束；表9–11的消融仅检验这些设计与结果的关联，在独立重训和多种子结果冻结前，不作因果归因或优效性结论。
