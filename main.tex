...
\hspace{2em}We retrospectively included 24,049 de-identified case records provided by a domestic data provider and multiple dental laboratories for method development. Each record comprised maxillary and mandibular intraoral scans, a three-dimensional preparation-margin curve, an FDI tooth designation, and a technician-designed reference CAD mesh. Eligible records had a scan field covering at least four adjacent teeth and a valid maxillomandibular occlusal relationship. Records were excluded if the mean closest-point distance from margin points to the reference mesh exceeded \(0.5\,\mathrm{mm}\), if more than \(2\%\) of the reference mesh lay outside the predefined local domain, or if the number of active FDG cells exceeded \(3\times10^6\). At the case-record level, records were partitioned by stratified random sampling according to FDI tooth position, jaw, and laterality into training, validation, and test sets in approximate proportions of \(85\%\), \(10\%\), and \(5\%\), respectively. The test set included posterior single-crown cases only.
