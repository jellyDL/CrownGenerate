...
表6汇总了各方法在完整测试集上的生成覆盖率，以及在共同可评价病例集（n = ...）上的几何、虚拟对颌与拓扑指标。本文完整模型的Chamfer-L1为 ... ± ... mm，HD95为 ... ± ... mm，F-score@0.10 mm为 ... ± ...%，颈缘线P95误差为 ... ± ... mm，相比DCrownFormer+分别改进 Δ = ... mm（95% CI：[..., ...]，p < ...）和 Δ = ... mm（95% CI：[..., ...]，p < ...）。生成完成率为 ...% (... / ...)，显著高于VBCD（...%）和FDI最近邻（...%）；平均法向角为 ... ± ...°（DCrownFormer+：... ± ...°，p < ...），印证了更优的表面平滑性和边界困难病例中的鲁棒性。
...
在虚拟咬合关系方面，本文完整模型的接触/近接触面积、穿透面积和穿透深度HD95均接近参考CAD水平，严重穿透病例率较DCrownFormer+降低 ... 个百分点（McNemar p < ...），提示多尺度条件对接触模式的有效约束。拓扑完整性方面，水密且绕向一致、单连通genus-0网格占比分别为 ...% 和 ...%，均显著超过DCrownFormer+（McNemar p < ... 和 p < ...）；非流形边、边界边和法向翻转发生率分别为 ...%、...% 和 ...%（见补充表S2）。
