...
图2. 定向有符号FDG的局部表示.（a）参考CAD仅在冠体表面邻域激活稀疏单元。（b）放大的活动单元记录柔性双网格顶点偏移 \(v_j\) 与轴向相交状态 \(b_j\)，并以同一活动坐标关联中心至最近表面的截断定向距离 \(d_j\) 和插值表面法向 \(n_j\)。（c）四类属性组成单元几何编码并输入稀疏几何VAE。非活动背景单元不存储几何；对于非水密参考网格，符号由最近表面法向定义为局部方向。
...
Feature阶段采用随机偏移的分层均匀时间采样，即 \(t_i=[u+(i-1)/B]\bmod 1\)，其中 \(B\) 为批量内病例数，\(i=1,\ldots,B\)，\(u\sim\mathcal U(0,1)\)。除流匹配速度误差外，训练目标还约束数据端估计 \(\hat x_0^F\) 及三个膨胀率下共18个轴向邻居的潜变量差分；后两项按 \(1-t\) 衰减，速度误差采用均值归一的四次时间权重 \(w_v(t)=(1+2t^4)/(1+2/5)\)。所有稀疏损失先在病例内汇总，再对病例等权平均。为在高曲率区域保持细节，各损失按参考潜变量的局部梯度能量加权，其构造与VAE细节项一致，权重与系数见表2。
...
稀疏VAE、Structure Flow和Feature Flow三阶段依次训练，网络骨干与训练框架详见Sec. 3.7；各阶段训练步数、批量大小、优化器及学习率见表1。
