...
该环节对应图1(c)中Structure Flow与Feature Flow的生成过程：Structure Flow由噪声生成结构潜变量并恢复为预测活动支持 \(\hat O\)；Feature Flow在 \(\hat O\) 上生成局部特征 \(\hat F\)；二者组成 \(\hat Z=(\hat O,\hat F)\) 后解码重建候选网格。根据第3.4节，Structure的牙科条件为 \(C^S=(C_S^g,C_{S,16}^v)\)；对坐标 \(q\)，Feature的牙科条件为 \(C^F(q)=(C_F^g,C_{F,16}^v,C_{F,64}^v(q),C_F^n(q))\)。Feature Flow还接收由活动支持导出的结构上下文，详见第3.5.3节。训练时，以0.1的病例级概率同时丢弃全部牙科条件，以学习无分类器引导（classifier-free guidance, CFG）的无条件分支[25]；Feature阶段的条件与无条件速度评估使用相同的活动支持 \(S\)，仅移除牙科条件（训练时 \(S=O\)，推理时 \(S=\hat O\)）。
...
其中 \(\sigma_{\min}=10^{-5}\)，\(t=0\) 和 \(t=1\) 分别对应近数据端与噪声端；当 \(\sigma_{\min}\to0\) 时，前者为数据端。\(C^\star\) 在Structure阶段取 \(C^S\)；在Feature阶段取完整坐标级上下文 \(\mathcal C^F(S)=\{(C^F(q),A(q;S))\mid q\in S\}\)，其中 \(A(q;S)\) 由活动支持导出，不属于牙科条件。

#### 3.5.2 Structure Flow

Structure Flow首先确定冠体潜变量的空间支持。VAE潜变量活动坐标被转换为 \(64^3\) 二值占位 \(O\)，再由冻结的TRELLIS结构编码器压缩为 \(16^3\)、8通道结构潜变量 \(x_0^S\)[8,9]。以 \(C^S=(C_S^g,C_{S,16}^v)\) 为输入，三维DiT经空间对齐适配和交叉注意力预测结构潜空间中的条件速度场，并由噪声生成 \(x_0^S\)。

推理时，冻结的结构解码器将生成的 \(x_0^S\) 还原为 \(64^3\) 占位分数，并据此确定Feature Flow的活动坐标。

#### 3.5.3 Feature Flow

给定活动支持 \(S\) 后，Feature Flow在其活动坐标上生成标准化的局部几何潜变量 \(x_0^F\)。训练与单阶段潜变量验证时 \(S=O\)，以避免Structure误差干扰局部特征学习；端到端生成可视化、网格评价与正式推理时 \(S=\hat O\)。对每个 \(q\in S\)，稀疏Feature DiT联合输入噪声潜变量、全局条件 \(C_F^g\)、体素读出 \(C_{F,64}^v(q)\)、坐标邻域条件 \(C_F^n(q)\) 以及由活动支持导出的稀疏轴向上下文 \(A(q;S)\)，并预测局部条件速度。在 \(16^3\) 条件标记尺度，\(C_F^g\) 与 \(C_{F,16}^v\) 构成交叉注意力标记；\(C_F^g\) 和从 \(64^3\) 场读取的 \(C_{F,64}^v(q)\) 经残差路径注入Feature token；\(C_F^n(q)\) 与 \(A(q;S)\) 共同调节局部特征。膨胀率为1、2和4的18个轴向邻居用于编码多尺度活动支持，潜在特征之间的交互由稀疏DiT完成。

![图3. Feature Flow的坐标级多尺度条件融合](../figures/feature_condition_detail.png)
图3. Feature Flow的坐标级多尺度条件融合（推理阶段）。（a）病例在局部坐标系中对齐后，以预测活动支持 \(\hat O\) 中的坐标 \(q\) 为查询位置；上、下颌、三维颈缘线和FDI牙位共同定义该位置的病例上下文。（b）\(C_F^g\) 和 \(C_{F,16}^v\) 构成交叉注意力标记；\(C_F^g\) 及其在 \(q\) 处从 \(64^3\) 场读取的体素读出 \(C_{F,64}^v(q)\) 经残差路径注入Feature token；\(C_F^n(q)\) 与由预测支持构建的稀疏轴向上下文 \(A(q;\hat O)\) 一并作用于该token。稀疏Feature DiT融合这些输入以预测局部几何特征 \(\hat F_q\)。紫色点划线表示牙科条件路径，蓝色实线表示活动支持与特征路径。
