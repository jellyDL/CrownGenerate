...
标准FDG能够描述局部几何和连接关系，但不显式编码表面两侧及朝向。为此，本文增加截断有符号距离和定向法向。计算前修复参考网格的不一致绕向；若水密网格的有符号体积为负，则整体反向，并在最近表面查询前删除退化三角面。设 \(p_j\) 为 \(x_j\) 在参考表面上的最近点，\(n_j^f\) 为最近三角面的单位法向，\(n_j\) 为该处重心插值并归一化的顶点法向，退化时置零。定向截断距离与活动单元特征定义为
