...
\hspace{2em}Early learning-based crown design methods commonly recast the 3D task as 2D occlusal-surface reconstruction. DentalRecNet employs depth projections along the occlusal direction and dual discriminators to reconstruct the defect region of mandibular first molars [3]. However, projection discards geometric information from axial surfaces, the cervical margin and occluded regions, while 2D metrics cannot adequately characterize the 3D relationship between a crown and the surrounding dentition.

\hspace{2em}Subsequently, Point-cloud, voxel-based and template-deformation methods instead model crowns directly in 3D while incorporating case-specific conditions. Hosseinimanesh et al.\ incorporated adjacent teeth, cervical margins and opposing dentition into point-cloud completion [1,5]. The DCrownFormer series predicted crown points and surface normals, followed by Poisson reconstruction or implicit refinement to obtain a mesh [4,21,34]. VBCD generated and refined crowns from voxelized intraoral scans using curvature, margin and FDI tooth-position cues [35], whereas MADCrowner used multiscale features for coarse-to-fine template deformation and constrained trimming with a predicted margin [33]. Nevertheless, sampling and resolution limits, surface reconstruction and template or margin errors can compromise local detail, boundary fidelity and topology.

\hspace{2em}Implicit-field and diffusion models have further extended dental completion. ToothDIT constructed individualized implicit templates from complete anterior-tooth morphology [2], whereas ToothCraft performed conditional diffusion over signed distance function (SDF) voxels [6]. These approaches often rely on anterior templates or synthetic data and remain constrained by SDF resolution and mesh conversion, limiting their ability to jointly satisfy the morphological and spatial constraints of real posterior crown cases with prepared abutments.
