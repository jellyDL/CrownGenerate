...
%% Text of abstract 
%% Todo：后期需要精简到 250 词内
Patient-specific dental crown morphology design is a critical step in the digital restoration of tooth defects. It requires the reconstruction of fine anatomical structures, including cusps, ridges, grooves, and fossae, while maintaining appropriate relationships with the cervical margin, adjacent teeth, and opposing dentition. Although computer-aided design systems have improved design efficiency, tooth-supported posterior single crowns still frequently require dental technicians to iteratively adjust standard templates. Existing learning-based methods also remain limited in representing high-resolution three-dimensional details and jointly modeling heterogeneous dental spatial conditions. To address these limitations, we propose a 3D dental crown generation framework based on an oriented signed flexible dual-grid representation and multiscale conditional flow matching. The framework comprises two components: sparse geometric representation and hierarchical conditional generation. The former jointly encodes crown morphology, signed distances, and oriented normals, and employs a sparse variational autoencoder to learn a compact latent representation. The latter sequentially generates the global occupancy structure and local surface features through a structure flow and a feature flow, respectively, thereby reconstructing candidate crown meshes. Complete maxillary and mandibular scans, manually delineated 3D cervical margin lines, and FDI tooth-position codes are incorporated into the generation process at global, voxel-aligned, and coordinate-neighborhood scales. Joint supervision of the decoded surfaces is further introduced to preserve occlusal details and surface continuity. Systematic comparisons with representative existing methods were conducted using an in-house dataset of existing dental crown designs. Quantitative results demonstrate that the proposed method generally outperforms the comparative methods in terms of geometric reconstruction quality and dental spatial compatibility. A source-blinded evaluation by experienced dental technicians further indicates superior overall design quality and laboratory usability. These findings suggest that the proposed framework improves the geometric consistency and laboratory usability of generated crown morphology, providing editable, patient-specific initial CAD designs for tooth-supported posterior single crowns.
\end{abstract}
...
%% Keywords
\begin{keyword}
Dental crown generation\sep Conditional flow matching \sep Flexible dual grids\sep Digital dental restoration
\end{keyword}
