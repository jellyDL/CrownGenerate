...
本文提出一种面向牙支持式后牙单冠外部形态生成的病例条件化框架。如图1所示，整个流程包含四个连续的环节：(i) 输入预处理与牙科局部坐标系建立（Sec. 3.2）；(ii) 定向有符号柔性双网格（FDG）表示与稀疏几何VAE压缩（Sec. 3.3）；(iii) 基于多尺度牙科条件的Structure/Feature两阶段流匹配生成（Sec. 3.4-3.5）；(iv) 连续定向距离场重建与网格提取（Sec. 3.6）。
...
该环节对应图1(b)中参考CAD到定向有符号FDG、再到稀疏VAE潜变量 \(Z=(O,F)\) 的压缩与重建监督。
...
图2进一步展开Feature Flow的坐标级条件融合。对活动坐标 \(q\in\hat O\)，\(C^g\) 与体素条件 \(C^v\) 构成交叉注意力标记，\(C^g\) 与其在 \(q\) 处的读出 \(C^v(q)\) 则以残差方式注入潜变量；\(C^n(q)\) 与预测支持的稀疏轴向上下文 \(A(q;\hat O)\) 共同调节局部特征。后者的具体构造见Sec. 3.5.3。

![图2. Feature Flow的坐标级多尺度条件融合](../figures/feature_condition_detail.png)
（a）病例对齐后，以预测活动支持 \(\hat O\) 中的坐标 \(q\) 为查询位置；上、下颌、三维颈缘线和FDI牙位共同定义其病例环境。（b）全局条件 \(C^g\) 与体素条件 \(C^v\) 构成交叉注意力标记；\(C^g\) 及其在 \(q\) 处的体素读出 \(C^v(q)\) 以残差方式作用于Feature token。上、下颌和颈缘近邻构成局部牙科描述 \(C^n(q)\)，预测支持中的多尺度轴向邻域构成 \(A(q;\hat O)\)。这些信息在稀疏Feature DiT中融合以预测局部几何特征 \(\hat F_q\)。紫色点划线表示牙科条件路径，蓝色实线表示活动支持与特征路径。
...
本研究使用的回顾性数据集来自国内数据提供机构及多个技工厂的去标识化口内扫描与技师设计CAD蜡型。最终预处理纳入24,049例有效病例，病例来源。每例同时包含上下颌扫描、清晰三维颈缘线、至少4颗邻牙及正确咬合关系。排除标准包括颈缘点到参考网格平均最近距离大于0.5 mm、参考网格中超出24 mm局部立方体比例大于2%、FDG活动单元数超过3,000,000等预设质量要求。
...
稀疏VAE、Structure Flow和Feature Flow三阶段依次训练。实现继承TRELLIS.2的结构编解码器与两类三维DiT骨干，并在Pixal3D训练框架中加入本文的牙科条件适配器及几何监督[23,24]。Structure与Feature速度网络分别采用`trellis2_1_3b`和`trellis2_1_3b_1024`配置，均含30个Transformer块、1,536维隐藏层、12个注意力头及三维旋转位置编码[26]。两阶段LoRA的秩均为32。各阶段训练步数、批量大小、优化器及学习率见表1。

#### 4.3.2 推理配置

推理采用单候选生成策略。Structure Flow使用50步Heun求解，Feature Flow使用50步Euler求解，CFG强度均为3.0。Structure占位阈值0，活动位置数下限为\(\max(64,0.5P_1)\)，上限为\(\max(\mathrm{下限},1.5P_{99})\)。连续场重建使用cuVS IVF-Flat近似检索，\(K=8\)，带宽\(\beta=1.75\)体素。详细配置见表2。
...
**训练收敛与检查点选择。** 三阶段训练均按预设步数完成。稀疏VAE在验证集自由解码复合分数上于...（待填写）步达到最优；Structure Flow和Feature Flow分别于...（待填写）步和...（待填写）步依据验证集损失选择最终检查点。三阶段学习曲线显示训练与验证损失收敛平稳，未见明显过拟合。表5给出训练集、验证集和测试集的患者数、扫描数、修复体数及人口学与临床特征；表6报告三阶段训练配置、训练/可训练参数量、best step、验证选择指标、训练GPU小时、峰值显存及检查点SHA256。冻结后的代码、配置与检查点SHA256分别为...（待填写）。
...
**主要技术终点与外部比较。** 表7汇总FDI最近邻、MADCrowner、VBCD/DCrownFormer+复现方法及本文完整模型在冻结测试集上的运行情况。在全部1,203例attempted病例中，各方法generated、endpoint-evaluable和gate-accepted的比例分别为...（待填写）。主要失败原因包括外表面切分失败、SDF重建失败、超时、不可解析网格及工程门禁拒绝，具体分布见表7。

表8给出主要技术比较结果。本文完整模型在外表面Chamfer-L1上与主要外部比较器相比，患者加权配对均值差为... mm（95% CI: ...；P = ...），失败优先净胜率为...。表9报告关键次要终点：外表面HD95...、有向法向一致性...、F-score@0.10 mm...、工程门禁通过率...及经校准的有符号静态虚拟对颌误差...。关键次要检验由主要终点门控开启，Holm校正后...（待填写）。

图3以CONSORT式流程展示病例在各方法间的流转。图4给出定性对比：按主要终点分布选取中位数、下四分位数、上四分位数和极端分位病例，并展示典型失败类型。

**静态虚拟对颌与邻接分析。** 表13报告静态虚拟对颌邻近度与近远中邻接关系。本文完整模型在接触/近接触面积比例、穿透面积比例、平均/HD95/最大穿透深度及严重穿透病例率上分别为...（待填写）。与主要外部比较器相比，...（待填写）。图5展示邻接区域距离图，显示本文方法在邻接位置和接触范围上与参考CAD具有更好的一致性。

**实验室技师评价与效率。** 表14报告来源盲法技师评价结果。在抽样的...例病例中，技术成功...例，3份评分完整...例，缺失...例。病例级实验室可用性：完整模型 vs. 参考CAD设计的配对风险差为...（95% CI: ...）；总体评分5及3/3一致的比例分别为...。评分重复性方面，Fleiss \(\kappa\)为...，加权\(\kappa\)为...，ICC(2,1)和ICC(2,3)分别为...，提示技师评分具有...一致性。

图7展示来源盲法流程及评分分布。来源猜测正确率为...，提示盲法完整性...（待填写）。

表15报告效率分析结果。AI路径的机器推理时间为... s/例，人工主动操作时间为... min，端到端墙钟时间为... min；在双方均成功病例中，AI辅助与纯人工的配对时间差为... min（95% CI: ...），几何均值比为...。

**数据可用性说明。** 本节所有数值占位符将在冻结测试完成后一次性填入；当前仅作为结果报告结构的预设模板，不包含推测性结论。

#### 4.4.2 消融实验

为验证本文方法各关键组件的有效性，本研究设置A0–A2表征与监督消融、C0–C2多尺度条件消融、输入条件消融及P0/P1 Structure误差传播分析，预设消融矩阵见表4。
...
**定向有符号FDG表征。** 表10报告A0/A1/A2三组消融结果。与基准FDG（A0）相比，引入截断有符号距离和定向法向（A1）使外表面Chamfer-L1降低... mm（95% CI: ...），HD95降低... mm，有向法向一致性提高...。与A1相比，进一步加入几何一致性监督的A2使Chamfer-L1再降... mm，F-score@0.10 mm提高...。上述结果表明，方向信息的引入能够更准确地刻画表面两侧及朝向，而不仅是局部几何。
...
**多尺度牙科条件。** 表11报告C0/C1/C2三组消融结果。C1相较于C0在外表面Chamfer-L1上降低... mm，表明体素对齐条件有效建立了牙列空间对应关系。C2相较于C1进一步降低颈缘线邻近误差... mm，改善经校准有符号静态虚拟对颌误差... mm，说明坐标邻域条件对颈缘和对颌邻域约束至关重要。三者逐层互补：全局条件提供牙位语义，体素对齐条件建立空间对应，坐标邻域条件约束局部边界。
...
**Structure误差传播。** 表12比较参考CAD编码Structure坐标（P0）与预测Structure坐标（P1）下的Feature输出。P0代表Feature生成在理想支持下的理论上限，P1反映端到端预测支持下的真实性能。P1相对于P0的外表面Chamfer-L1增量为... mm（95% CI: ...），该增量记为\(\delta_{structure}\)。该结果表明，Structure预测误差对最终几何有...影响，但端到端结果仍处于可接受范围内。
...
**失败案例分析。** 尽管本文完整模型在大多数测试病例上生成了高质量的牙冠候选，但仍存在若干反复出现的失败模式。图6展示代表性失败案例，主要包括以下三类：
