...
图1给出了病例条件化牙冠生成框架。对病例 \(i\)，条件输入 \(C_i=(J_i^u,J_i^l,M_i,f_i)\) 包括同一静态咬合坐标系下的上下颌扫描、人工标注的有序三维颈缘线和FDI牙位。训练时另提供技师设计的参考CAD蜡型网格 \(S_i\)；推理时仅使用 \(C_i\) 生成病例适配的可编辑候选网格 \(\hat S_i\)。

框架由三条相互衔接的路径组成。首先，牙科条件路径对输入进行局部坐标对齐与采样，并构建用于Structure和Feature阶段的多尺度条件。其次，训练阶段的几何表示路径将 \(S_i\) 转换为定向有符号FDG，再由稀疏几何VAE编码为潜变量 \(Z_i=(O_i,F_i)\)，其中 \(O_i\) 为活动支持，\(F_i\) 为其上的局部几何特征。最后，生成与重建路径依次预测活动支持和局部特征，并通过冻结的VAE与连续定向场恢复三角网格。为分层建模冠体整体结构与局部形态，条件分布分解为
...
其中，\(C_i^S\) 包含全局与体素对齐条件，\(C_i^F\) 进一步加入坐标邻域条件。Structure Flow根据 \(C_i^S\) 建模 \(O_i\)，Feature Flow在给定 \(O_i\) 时结合 \(C_i^F\) 建模 \(F_i\)。推理时二者生成 \(\hat Z_i=(\hat O_i,\hat F_i)\)，经解码与网格重建得到 \(\hat S_i\)。该输出仅作为牙支持式后牙单冠外部形态的CAD初稿；为形成闭合工程网格而产生的区域不代表临床冠内表面，本研究亦不建模粘固间隙、制造补偿或加工参数。
