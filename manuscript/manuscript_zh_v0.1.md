...
**主要几何结果。** 本文在统一测试协议下比较完整模型、FDI牙位最近邻基线、MADCrowner[33]、VBCD[35]和DCrownFormer+[34]。共同可评价的\(n=...\)例病例中，完整模型与预设主要比较器的外表面Chamfer-L1分别为...和... mm，病例内配对差值为... mm（95% CI：...）。表6同时报告关键次要几何指标，其余预设指标见补充材料。

**表6. 冻结测试集共同可评价病例的几何结果**

| 方法 | Chamfer-L1（mm）↓ | HD95（mm）↓ | 有向法向一致性↑ | F-score@0.10 mm↑ | 颈缘线平均/P95误差（mm）↓ |
|---|---|---|---|---|---|
| FDI最近邻 | ... | ... | ... | ... | .../... |
| MADCrowner | ... | ... | ... | ... | .../... |
| VBCD | ... | ... | ... | ... | .../... |
...
**表7. 各方法的生成、可评价与门禁通过情况**

| 方法 | generated，\(n/N\)（%） | endpoint-evaluable，\(n/N\)（%） | gate-accepted，\(n/N\)（%） |
|---|---|---|---|
| FDI最近邻 | ... | ... | ... |
| MADCrowner | ... | ... | ... |
| VBCD | ... | ... | ... |
...
**静态虚拟对颌与拓扑结果。** 表8报告接触/近接触面积、穿透程度和拓扑完整性。完整模型与主要比较器的穿透面积差值为...个百分点（95% CI：...），严重穿透病例率差值为...个百分点（95% CI：...）。接触面积不作单向优劣解释，而应结合参考CAD分布及穿透结果判断；图5以统一色标展示代表病例的有符号距离分布。

**表8. 各方法的静态虚拟对颌与拓扑结果**

| 方法 | 接触/近接触面积（%） | 穿透面积（%）↓ | 平均/HD95/最大穿透深度（mm）↓ | 严重穿透病例率（%）↓ | 水密率（%）↑ | 单连通genus-0率（%）↑ |
|---|---|---|---|---|---|---|
| FDI最近邻 | ... | ... | .../.../... | ... | ... | ... |
| MADCrowner | ... | ... | .../.../... | ... | ... | ... |
| VBCD | ... | ... | .../.../... | ... | ... | ... |
...
<!-- 定稿前需在方法部分补充技师评价的样本量、评分量表、评审者数量和统计方案。 -->
**技师评价与效率。** 表9报告来源盲法技师评价与病例内配对设计时间。人工路径与完整模型的解剖形态评分差值为...（95% CI：...），设计时间差值为... min（95% CI：...）；总体评分者间一致性为...，来源猜测正确率为...%（95% CI：...）。图6展示配对评分分布与盲法检查结果。

**表9. 技师评价与设计效率**

| 指标 | 人工路径 | 本文完整模型 | 配对差值（95% CI） |
|---|---|---|---|
| 解剖形态评分 | ... | ... | ... |
| 邻接与咬合协调评分 | ... | ... | ... |
| 修改需求评分 | ... | ... | ... |
...
**定性结果。** 图4在预先确定的同一组病例和统一视角下比较参考CAD、外部方法与完整模型，并展示整体冠体、牙尖/沟窝和颈缘局部细节；图5以统一色标展示相同病例的有符号对颌距离。定性结果用于辅助解释表6和表8，不替代定量比较。
 
#### 4.4.2 消融实验

为验证本文方法各关键组件的有效性，本研究设置A0–A2表征与监督消融、C0–C2多尺度条件消融、输入条件消融及P0/P1 Structure误差传播分析，预设消融矩阵见表10。
...
**定向有符号FDG表征。** 表11报告A0/A1/A2三组消融结果。与基准FDG（A0）相比，引入截断有符号距离和定向法向（A1）使外表面Chamfer-L1降低... mm（95% CI: ...），HD95降低... mm，有向法向一致性提高...。与A1相比，进一步加入几何一致性监督的A2使Chamfer-L1再降... mm，F-score@0.10 mm提高...。上述结果表明，方向信息的引入能够更准确地刻画表面两侧及朝向，而不仅是局部几何。

**表11. A0/A1/A2 表征与监督消融结果**
...
**多尺度牙科条件。** 表12报告C0/C1/C2三组消融结果。C1相较于C0在外表面Chamfer-L1上降低... mm，表明体素对齐条件有效建立了牙列空间对应关系。C2相较于C1进一步降低颈缘线邻近误差... mm，改善经校准有符号静态虚拟对颌误差... mm，说明坐标邻域条件对颈缘和对颌邻域约束至关重要。三者逐层互补：全局条件提供牙位语义，体素对齐条件建立空间对应，坐标邻域条件约束局部边界。

**表12. C0/C1/C2 多尺度条件消融结果**
...
**Structure误差传播。** 表13比较参考CAD编码Structure坐标（P0）与预测Structure坐标（P1）下的Feature输出。P0代表Feature生成在理想支持下的理论上限，P1反映端到端预测支持下的真实性能。P1相对于P0的外表面Chamfer-L1增量为... mm（95% CI: ...），该增量记为\(\delta_{structure}\)。该结果表明，Structure预测误差对最终几何有...影响，但端到端结果仍处于可接受范围内。

**表13. P0/P1 Structure 误差传播结果**
...
**结果解释。** 完整模型的表现应由共同可评价病例中的主要几何终点、运行完整性和静态对颌/拓扑指标共同解释（表6–8），不能以单一案例或各自门禁通过子集替代总体结论。定向有符号FDG、Structure/Feature分层生成和多尺度牙科条件分别面向局部几何编码、整体支持预测以及颈缘与对颌约束；表11–13的消融仅检验这些设计与结果的关联，在独立重训和多种子结果冻结前，不作因果归因或优效性结论。
