...
在相同病例划分、预处理和质量检查流程下，本文将完整模型与五种对比方法进行定量评估：FDI牙位最近邻基线、MADCrowner[33]、VBCD[35]、DCrownFormer+[34]和本文提出的方法。所有方法完成原生推理和必要的表面重建后，均采用第4.2节规范的外表面切割、距离计算和拓扑检查流程评估。

表6报告了各方法在完整测试集上的生成覆盖率，以及在所有方法共同可评价病例集上的主要几何指标（n = ...）。本文完整模型的Chamfer-L1距离为 ... ± ... mm，HD95为 ... ± ... mm，F-score@0.10 mm为 ... ± ...%，颈缘线P95邻近误差为 ... ± ... mm。相比主要对比方法DCrownFormer+（Chamfer-L1：... ± ... mm；HD95：... ± ... mm），本文方法的配对差异分别为 Δ = ... mm（95% CI：[..., ...]）和 Δ = ... mm（95% CI：[..., ...]），经Wilcoxon配对秩和检验显著性为 p < ...。完整模型的生成完成率为 ...% (... / ...)，高于基线方法。

**表6. 各方法的生成覆盖与主要几何指标**

| 方法 | 生成完成率 | Chamfer-L1 (mm) | HD95 (mm) | F-score@0.10 mm | 颈缘线P95误差 (mm) |
|:---|:---|:---:|:---:|:---:|:---:|
| FDI最近邻 | ...% | ... ± ... | ... ± ... | ... ± ... | ... ± ... |
| MADCrowner | ...% | ... ± ... | ... ± ... | ... ± ... | ... ± ... |
| VBCD | ...% | ... ± ... | ... ± ... | ... ± ... | ... ± ... |
...
*注：* 生成完成率定义为推理链路输出可解析网格的病例比例，独立于后续质量门禁，以完整测试集为分母。几何指标为平均值±标准差，在全部方法共同终点可评价的病例集上计算（n = ...）。Chamfer-L1和HD95为对称双向距离，单位为mm。F-score@0.10 mm为在0.10 mm阈值下的平衡精度指标。颈缘线P95误差为颈缘线上95百分位数处的点到预测网格的最近距离。其余预设指标（法向一致性、曲率、二面角等）见补充表S1。

进一步的定向法向一致性分析表明，本文方法在平均法向角为 ... ± ...°，显著优于DCrownFormer+的 ... ± ...°（p < ...）。在局部法向变化方面（六邻域离散Laplacian计算），本文方法的均值为 ... ± ...°/mm，相比对比方法的 ... ± ...°/mm，体现了更好的表面平滑性和解剖细节保留。

##### 空间关系与拓扑完整性评估

除表面几何一致性外，本文进一步评估冠体与对颌牙的空间关系和生成网格的拓扑质量，以反映临床可用性和设计合理性。表7汇总了各方法的虚拟对颌接触特征和拓扑指标。

**表7. 各方法的虚拟对颌关系与拓扑指标**

| 方法 | 接触/近接触 (%) | 穿透面积 (%) | 穿透深度 HD95 (mm) | 严重穿透病例 | 水密一致 | Genus-0单连通 |
|:---|:---:|:---:|:---:|:---:|:---:|:---:|
| 参考CAD | ...% | ...% | ... ± ... | ...% | ...% | ...% |
| FDI最近邻 | ...% | ...% | ... ± ... | ...% | ...% | ...% |
| MADCrowner | ...% | ...% | ... ± ... | ...% | ...% | ...% |
...
*注：* 接触/近接触（0–0.10 mm间隙）面积占临床外表面的比例；穿透为负间隙，按绝对值计算面积占比。穿透深度HD95为穿透区域内第95百分位数处的穿透深度。严重穿透病例定义为穿透深度超过0.10 mm的病例比例。水密一致为同时满足水密性和绕向一致性的网格比例。Genus-0单连通为拓扑简单（欧拉数=2，无孔）的网格比例。对参考CAD，上述指标作为临床设计的解释性参考，不参与方法排序。严重穿透、边界缺陷等拓扑指标仅统计于生成完成的网格（DCrownFormer+、本文：n = ...）。

本文完整模型的接触/近接触面积为 ...%，穿透面积为 ...%，穿透深度HD95为 ... ± ... mm。与参考CAD相比（接触/近接触：...%，穿透面积：...%），表明本文方法在虚拟对颌空间关系上接近设计标准。严重穿透（>0.10 mm）的病例率为 ...% (... / ...)，相比DCrownFormer+的 ...% (... / ...) 降低了 ... 个百分点。

在拓扑完整性方面，本文方法生成的水密且绕向一致网格占比为 ...% (... / ...)，单连通genus-0网格占比为 ...% (... / ...)，均超过对比方法。非流形边、边界边和法向翻转等拓扑缺陷的发生率分别为 ...%、...% 和 ...%（见补充表S2），提示生成网格的拓扑质量更好，降低了后续CAD修正的需求。

##### 定性分析与典型案例

图4展示了三个代表性病例中各方法生成冠体的整体形态（左列）、咬合沟窝细节（中列）和颈缘区域（右列）。与参考CAD的表面距离误差（色标：蓝色为吻合 <0.05 mm，绿色为0.05–0.10 mm，黄色为0.10–0.20 mm，红色为 >0.20 mm）直观反映了方法间的几何差异。本文方法在牙尖、中央沟和颈缘处显示更均匀的误差分布和更少的局部伪影，而对比方法常出现颈缘不连续或咬合面坡度异常。
...
图5报告了来源盲法下技师对人工设计路径和本文完整模型生成冠体的修改需求评分。每个病例由3名认证技师独立评分（Likert五级：无需、轻度、中度、重度、无法使用），采用多数表决或讨论确定最终等级。统计结果表明，本文完整模型的修改需求分布为无需修改 ...%、轻度修改 ...%、中度修改 ...%、重度修改 ...%，相比人工设计路径（无需修改 ...%、轻度修改 ...%、中度修改 ...%、重度修改 ...%），表明模型生成的初稿具有可比的临床适用性。

评分者间一致性（Fleiss' κ）为 ...（95% CI：[..., ...]），表示中等至良好的一致性。技师对生成来源的盲猜测准确率为 ...%，无显著高于随机的证据（χ² = ..., p = ...），说明两种方案的设计质量难以区分。平均修改所需时间，本文模型初稿为 ... ± ... 分钟，相比人工设计路径的 ... ± ... 分钟，节省时间比例为 ...%，提示模型具有实际的临床效率优势。

![图5. 技师盲评的修改需求分布。环形图分别表示人工设计路径（左）和本文完整模型（右）的病例级评分分布；扇区面积代表各等级病例的比例，中心数字为有效评价病例数（n = ...）。无需修改（绿）、轻度修改（黄）、中度修改（橙）和重度修改（红）的比例直观反映了两种方案的临床适用性。](../figures/figure5_technician_score_distribution_template.png)
...
为验证完整模型各关键设计组件对最终性能的贡献，本文开展了系统的消融研究。研究涵盖四个维度：（1）定向有符号距离与法向表征的有效性；（2）联合几何监督对表面细节和颈缘连续性的影响；（3）多尺度牙科条件（全局、体素对齐和坐标邻域）的递进作用；（4）三维颈缘线、对颌牙列和FDI牙位等临床输入的必要性；（5）Structure生成的支持预测对下游Feature生成的误差传播。

##### 消融实验设计

**统一评估协议。** 所有消融采用相同的数据划分（表1中冻结的训练/验证/测试集）、输入预处理（第3.2节）、单候选推理（seed=2026+病例ID）和质量检查流程（第4.3节）。几何与静态虚拟对颌指标均在共同终点可评价病例集（n = ...）上进行病例内配对比较，采用Wilcoxon配对秩和检验（双尾，α = 0.05）评估配对差异的显著性；F-score等比率指标使用McNemar检验。除Structure误差传播（消融H）仅作诊断用途外，每个对照设计遵循单因素变化原则，仅改变一个预设因素。未生成或质量门禁不通过的病例统计为运行覆盖不足，独立报告。

完整配置记为 \(F_{\mathrm{full}} = \mathrm{A2} = \mathrm{C2} = \mathrm{P1}\)，对应同一冻结运行中的定向有符号表征（A2）、联合几何监督、全部多尺度牙科条件（C2）和预测Structure支持（P1）。

**变体训练与冻结策略。** 
- **A0–A2 表征/监督变体**：独立重训稀疏VAE和完整生成链路（Structure+Feature Flow），每个变体训练10个独立随机种子以估计跨种子的稳定性。
- **C0–C2 多尺度条件变体**：固定A2冻结的VAE后端，独立重训Structure DiT、Feature DiT和相应条件适配器。  
- **I-Margin、I-Opposing、I-FDI 输入消融**：固定A2 VAE后端，在训练和推理的全部尺度移除相应条件（颈缘线、对颌牙列或FDI牙位），独立重训完整生成链路。
- **P0/P1 支持来源**：不重新训练，使用同一 \(F_{\mathrm{full}}\) 冻结检查点，固定病例级随机性和推理配置，仅在Feature阶段替换支持来源（P0：参考CAD导出的oracle支持；P1：Structure Flow预测的支持），以诊断Structure误差传播。

表8列出各消融对照的关键参数、执行方式和评估指标。
...
*注：* CD_L1 = Chamfer-L1；种子数表示独立训练运行次数。A0–A2消融使用10个种子以估计跨种子稳定性；C/I消融使用3个种子以测试变异性。所有配对比较在相同病例集（n = ...）上进行，采用Wilcoxon秩和检验（双尾）或McNemar检验（二分变量）报告显著性。

##### 4.4.2.1 定向有符号表征与表面监督

**消融A：定向有符号表征的有效性。** 对照组A0采用标准FDG（仅顶点偏移与轴向相交状态），实验组A1在保持VAE架构、训练数据和Structure/Feature Flow不变的前提下增加截断有符号距离和定向法向。十个独立种子的结果汇总于表9。
...
**消融B：联合几何监督的必要性。** 对照组A1采用基础重建损失（顶点、距离、法向的点态MSE）；实验组A2额外引入梯度一致性、离散Laplacian、距离-法向线积分和高频细节等监督项（第3.6.1节）。十个独立种子的结果见表9。
...
C2相对C1的颈缘线P95邻近误差降低 Δ = ... mm（95% CI：[..., ...]，p < ...），穿透面积减少 ... 个百分点（p < ...）。坐标邻域条件的引入使Feature Flow能够根据局部几何环境（邻牙距离、颈缘方向）动态调整冠体细节，特别是在颈缘过渡区和邻接区，显著改进了与对颌和邻牙的空间协调。三阶段条件递进的系统效应见表10。

**表10. 多尺度条件的递进效应**（n = ..., 每配置3个种子）
...
**表11. Structure支持来源的误差传播分析**（n = ..., oracle诊断）
...
**计算开销与效率。** 相比消融对照，完整模型（\(F_{\mathrm{full}}\)）的计算成本变化汇总于表12。多尺度条件注入和坐标邻域查询增加了特征编码的计算量，但由于稀疏结构设计，额外开销相对可控。单例推理时间主要受Feature阶段最多处理32,768个活动位置的限制；坐标邻域查询（每位置4×3近邻）在GPU上并行化后，相对基础Feature Flow的增加不超过 ...%。
