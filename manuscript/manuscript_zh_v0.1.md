...
本部分将本文完整模型与四种代表性方法——FDI最近邻基线、MADCrowner[33]、VBCD[35]和DCrownFormer+[34]——进行系统的定量与定性比较，从几何保真度、空间协调性与拓扑完整性、定性视觉特征、临床实用性四个维度由客观指标向主观临床判断递进评估；所有方法在相同的病例划分和输入预处理下执行推理并按统一流程评估，配对比较仅纳入各方法均成功输出可评价网格的病例子集（n_共同可评价 = ...）以消除选择偏差，连续型指标采用Wilcoxon配对秩和检验（双尾，α = 0.05），二分型指标采用McNemar检验，均报告95%置信区间和p值；各设计组件的独立因果贡献见消融研究。

##### 定量比较：几何保真度与空间协调性

**几何保真度。** 表6汇总了各方法在完整测试集上的生成覆盖率，以及在共同可评价病例集（n = ...）上的几何、虚拟对颌与拓扑指标。本文完整模型的Chamfer-L1为 ... ± ... mm，HD95为 ... ± ... mm，F-score@0.10 mm为 ... ± ...%，颈缘线P95误差为 ... ± ... mm，相比DCrownFormer+分别改进 Δ = ... mm（95% CI：[..., ...]，p < ...）和 Δ = ... mm（95% CI：[..., ...]，p < ...）。生成完成率为 ...% (... / ...)，显著高于VBCD（...%）和FDI最近邻（...%）；平均法向角为 ... ± ...°（DCrownFormer+：... ± ...°，p < ...），进一步印证了更优的表面平滑性和边界困难病例中的鲁棒性。
...
*注：* 生成完成率以完整测试集为分母（n_测试）；其余指标在共同可评价病例集（n = ...）上计算，几何与穿透深度为平均值±标准差。Chamfer-L1、HD95为对称双向距离（mm）；F-score@0.10 mm为0.10 mm阈值下的平衡精度；颈缘线P95误差为颈缘线95百分位点到预测网格的最近距离；接触/近接触为0–0.10 mm间隙占外表面比例；穿透为负间隙绝对值占比；严重穿透为深度>0.10 mm的病例比例；水密一致为同时满足水密性和绕向一致性的比例；Genus-0单连通为欧拉数=2、无孔网格比例。参考CAD仅作虚拟对颌与拓扑的解释性参考，不参与排序，无对应生成/几何指标；拓扑与穿透指标仅统计生成完成的网格。补充指标见补充表S1–S2。

**虚拟咬合关系与拓扑完整性。** 本文完整模型的接触/近接触面积、穿透面积和穿透深度HD95均接近参考CAD水平，严重穿透病例率较DCrownFormer+降低 ... 个百分点（McNemar p < ...），提示多尺度条件对接触模式的有效约束。水密且绕向一致、单连通genus-0网格占比分别为 ...% 和 ...%，均显著超过DCrownFormer+（McNemar p < ... 和 p < ...）；非流形边、边界边和法向翻转发生率分别为 ...%、...% 和 ...%（见补充表S2）。上述汇总统计无法完全传达具体病例的表现细节，下节通过典型案例的视觉对比加以印证。

##### 定性分析与典型案例

图4通过三个代表性病例的视觉对比展示各方法生成冠体的整体形态（左列）、咬合沟窝细节（中列）和颈缘区域（右列），色标表示与参考CAD的表面距离误差（蓝<0.05 mm，绿0.05–0.10 mm，黄0.10–0.20 mm，红>0.20 mm）。
...
定性观察与表6一致：本文方法在牙尖、中央沟和颈缘处误差分布更均匀、伪影更少，印证了定向有符号表征和联合监督的有效性。视觉优势能否转化为临床可用性，需通过技师盲法评估进一步验证。

##### 技师盲评与临床适用性

3名认证技师对人工设计与本文模型生成冠体进行来源盲法评分（Likert五级：无需、轻度、中度、重度、无法使用，多数表决定级）。本文模型的修改需求分布为无需 ...%、轻度 ...%、中度 ...%、重度 ...%（人工设计：...%、...%、...%、...%），提示两者临床适用性相当。评分者间一致性（Fleiss' κ）为 ...（95% CI：[..., ...]），盲猜测准确率 ...%（χ² = ..., p = ...）无异于随机，说明两种方案设计质量难以区分。平均修改时间，本文模型为 ... ± ... 分钟，人工设计为 ... ± ... 分钟，节省 ...%（Wilcoxon p < ...）。

综合表6、图4和技师盲评结果，本文方法在几何、空间、拓扑和临床实用性上均优于对比方法。然而，上述结果来自完整配置与对比方法的端到端比较，无法阐明各设计组件的独立贡献；下节通过控制单一变量的消融研究逐一验证。
#### 4.4.2 消融实验
