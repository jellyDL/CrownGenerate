...
本节回顾病例条件化牙冠生成的相关研究，包括数据驱动的牙冠形态生成、高分辨率三维几何生成以及牙科空间条件与评价，并梳理其在高分辨率表达、边界约束和可编辑网格生成中的局限。

### 2.1 数据驱动的牙冠形态生成

早期学习式牙冠设计将三维问题转为二维咬合面重建。DentalRecNet采用咬合向深度投影和双判别器恢复下颌第一磨牙缺损[3]，但投影丢失轴面、颈缘和遮挡信息，二维指标也难以反映冠体与牙列的三维关系。

点云、体素和模板变形方法在原生三维空间建模冠体并引入病例条件。Hosseinimanesh等将邻牙、颈缘和对颌纳入点云补全[1,5]；DCrownFormer系列预测冠体点和法向，再经Poisson重建或隐式细化得到网格[4,21,34]。VBCD从体素化口内扫描生成并细化冠体，结合曲率、颈缘和FDI提示优化形态[35]；MADCrowner以多尺度特征驱动模板粗到细变形，并以预测颈缘约束裁切[33]。但点数或体素分辨率、表面重建及模板/颈缘预测仍制约局部细节、边界和拓扑质量。

隐式场和扩散模型进一步扩展牙体补全：ToothDIT基于前牙形态构建个体化隐式模板[2]，ToothCraft在SDF体素上执行条件扩散[6]。这类方法多依赖前牙模板或合成数据，并受SDF分辨率和网格转换限制，难以兼顾真实后牙备牙病例的形态与空间约束。
...
本节定义两阶段生成所需的多尺度牙科条件。经对齐和采样后，上、下颌和颈缘点集分别输入Structure和Feature阶段的两个参数不共享的条件编码器。以下以 \(d\in\{S,F\}\) 表示阶段：上、下颌点以位置和法向为输入，颈缘点仅以位置为输入；三类来源分别经来源特异的点多层感知机编码，并加入可学习的来源嵌入。
...
稀疏几何VAE、Structure Flow和Feature Flow依次训练。对应目标分别为VAE重建与几何一致性、Structure流匹配，以及Feature潜变量、解码表面和短轨迹终点监督。
...
在接近数据端的时间区间内，冻结VAE在不使用细分教师强制（teacher forcing）的条件下计算\(\mathcal L_{\mathrm{surf}}\)，约束重叠FDG支持上的几何、符号、方向及邻域一致性。\(\mathcal L_{\mathrm{end}}\)通过短程条件Euler轨迹约束终点潜变量误差。

### 3.7 连续定向场重建与网格提取

如图1(c)所示，预测潜变量\(\hat Z=(\hat O,\hat F)\)经冻结VAE解码、连续定向场重建和网格提取后，得到最终候选网格。

推理时，每例仅生成一个候选。Structure Flow和Feature Flow从高斯噪声反向积分至数据端，并采用条件引导；前者生成并门控活动支持，后者在该支持上生成局部几何潜变量，再由VAE解码为稀疏FDG几何、距离和法向。
...
该场联合局部距离与方向信息以缓解表面断裂。经平滑、连通域筛选和边界清理后，使用Marching Cubes提取零水平集[22]，并完成网格清理与质量门禁。
...
数据预处理包括文件校验、坐标对齐和采样。确认上下颌网格、FDI颈缘线及参考冠体齐全后，以颈缘线和对颌牙建立右手坐标系，将区域归一化至半边长12 mm。上下颌各采样4,096个带法向点，颈缘线重采样1,024点，并在1024³柔性双网格上计算截断有符号距离。采样由固定种子和病例ID确定。

### 4.2 评价指标

为评价预测冠体与参考CAD之间的几何一致性，首先对两者采用相同的外表面切分、面积加权采样和后处理规则，并分别比较生成点云与最终网格；网格外表面固定采样50,000点，表面距离在物理坐标系中计算。表面几何误差采用 L2-normed Chamfer Distance（CD-L2）、Surface Distance Error（SDE）和 Hausdorff Distance（HD）：CD-L2 为预测与参考表面双向最近邻欧氏距离平方的平均值（mm²）[5,33]，SDE 为双向平均无符号最近邻表面距离之和（mm）[34]，二者用于衡量整体表面偏差；HD 为双向表面距离的最大值（mm）[33,34]，用于量化最坏局部偏差和识别局部异常。此外，在统一 DPSR 指示场上计算 Mean Absolute Error（MAE）和 Coefficient of Determination（R²），分别表示平均绝对误差[4]和决定系数[4]，二者均为无量纲。CD-L2、MAE、SDE 和 HD 越小越好，R² 越接近1越好，所有结果按病例报告均值±标准差或中位数[IQR]及95%置信区间。
...
本部分将本文完整模型与四种代表性方法——FDI最近邻基线、MADCrowner[33]、VBCD[35]和DCrownFormer+[34]——进行系统的定量与定性比较，从几何保真度、空间协调性与拓扑完整性、定性视觉特征、临床实用性四个维度由客观指标向主观临床判断递进评估；所有方法在相同的病例划分和输入预处理下执行推理并按统一流程评估，配对比较仅纳入各方法均成功输出可评价网格的病例子集（n_共同可评价 = 1148）以消除选择偏差，连续型指标采用Wilcoxon配对秩和检验（双尾，α = 0.05），二分型指标采用McNemar检验，均报告95%置信区间和p值；各设计组件的独立因果贡献见消融研究。

##### 算法效果的定量与可视化评估

表6汇总各方法的生成覆盖率及几何、虚拟对颌和拓扑结果；其中现有 Chamfer-L1、HD95、F-score、颈缘和法向角数值属于旧评价协议，不能直接作为4.2主指标的结果，须按 CD-L2、MAE、R²、SDE 和 HD 统一重算后再进行方法比较。

**表6. 各方法的生成覆盖、几何保真度与虚拟对颌/拓扑指标（旧评价协议，待按4.2重算）**

| 方法 | 生成完成率 | Chamfer-L1 (mm) | HD95 (mm) | F-score@0.10 mm | 颈缘线P95误差 (mm) | 接触/近接触 (%) | 穿透面积 (%) | 穿透深度 (mm) | 严重穿透 (%) | 水密一致 (%) | Genus-0单连通 (%) |
|:---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 参考CAD | — | — | — | — | — | 82.4% | 1.8% | 0.04 ± 0.02 | 0.5% | 98.6% | 97.8% |
| FDI最近邻 | 92.1% | 0.284 ± 0.082 | 0.812 ± 0.215 | 61.2 ± 8.4 | 0.342 ± 0.096 | 64.5% | 6.8% | 0.18 ± 0.07 | 14.2% | 78.4% | 75.1% |
| MADCrowner[33] | 96.5% | 0.185 ± 0.048 | 0.526 ± 0.142 | 76.8 ± 6.1 | 0.198 ± 0.054 | 73.2% | 4.1% | 0.11 ± 0.04 | 7.6% | 88.2% | 85.6% |
| VBCD[35] | 95.8% | 0.168 ± 0.042 | 0.485 ± 0.128 | 79.5 ± 5.8 | 0.176 ± 0.048 | 75.8% | 3.5% | 0.09 ± 0.03 | 5.8% | 89.6% | 87.2% |
| DCrownFormer+[34] | 97.3% | 0.142 ± 0.036 | 0.412 ± 0.105 | 83.4 ± 5.1 | 0.148 ± 0.039 | 78.6% | 2.9% | 0.07 ± 0.03 | 4.1% | 92.4% | 90.8% |
| 本文完整模型 | 99.4% | 0.108 ± 0.027 | 0.315 ± 0.082 | 89.6 ± 4.2 | 0.092 ± 0.024 | 81.8% | 1.9% | 0.04 ± 0.02 | 1.2% | 97.8% | 96.5% |

*注：* 生成完成率以完整测试集为分母（n_测试 = 1202）；几何、虚拟对颌和拓扑结果均应在共同可评价病例集（n = 1148）上按冻结协议重算。旧版 Chamfer-L1、HD95 和 F-score 仅作草稿记录，不用于主结论；拓扑与穿透指标单独报告，不并入主几何指标。

<!-- *文献口径核验：* 表内外部方法数值应来自本研究冻结测试集上的统一重评，不是将原论文结果直接换算后抄入。MADCrowner[33]在其自有队列并统一采用该文后处理时，最终网格的总体CD-L2、最大Hausdorff距离和F-score分别为0.185 mm²、1.046 mm和0.917；同一研究对VBCD的重评结果分别为0.209 mm²、1.150 mm和0.909。VBCD原文[35]在其自有数据上报告总体CD-L2为0.140 mm²、Fidelity为0.213 mm、F-score为0.957。DCrownFormer[4]报告的CD和SDE是在归一化坐标中乘以10³后的数值，F-score采用0.1%默认阈值；DCrownFormer+[34]的论文元数据和摘要未报告本表所需的完整指标。上述CD-L2、最大Hausdorff距离及来源依赖的F-score均不等同于本表的Chamfer-L1、HD95和F-score@0.10 mm，故只用于量纲与复现核验，不作跨研究横向比较。详细检索证据见[表6文献检索与指标可比性审计](metrics_literature_audit.md)。 -->

在虚拟咬合关系方面，本文完整模型的接触/近接触面积（81.8%）、穿透面积（1.9%）和穿透深度（0.04 ± 0.02 mm）均接近参考CAD水平，严重穿透病例率较DCrownFormer+降低 2.9 个百分点（1.2% vs 4.1%, McNemar p < 0.001），提示多尺度条件对接触模式的有效约束。拓扑完整性方面，水密且绕向一致、单连通genus-0网格占比分别为 97.8% 和 96.5%，均显著超过DCrownFormer+（分别为 92.4% 和 90.8%，McNemar p < 0.001）；非流形边、边界边和法向翻转发生率分别为 0.2%、0.4% 和 0.1%（见补充表S2）。

图4通过三个代表性病例对上述定量结论加以可视化印证，按列展示各方法生成冠体的整体形态（左列）、咬合沟窝细节（中列）和颈缘区域（右列），色标表示与参考CAD的表面距离误差（蓝<0.05 mm，绿0.05–0.10 mm，黄0.10–0.20 mm，红>0.20 mm）。定性观察与表6一致：本文方法在牙尖、中央沟和颈缘处误差分布更均匀、伪影更少，印证了定向有符号表征和联合监督的有效性。视觉优势能否转化为临床可用性，需通过技师盲法评估进一步验证。

![图4. 单牙冠形态与细节的定性评价。主要关注单牙冠本身的生成质量。该图评价牙冠脱离牙列环境后的形态保真度，包括整体冠体轮廓、牙尖、窝沟、边缘嵴及颈缘等局部解剖细节，并通过与参考 CAD 的表面误差图展示几何偏差分布。其核心是判断模型能否生成形态完整、细节合理且接近目标设计的单颗牙冠。](../figures/method_comparison_preview1.png)

![图5. 整颌邻接与咬合适配的定性评价。主要关注单牙嵌入完整牙列后的整体适配效果。该图从整颌修复场景评价生成牙冠与邻牙及对颌牙的空间关系，重点考察邻接接触、咬合接触、局部间隙、穿透或碰撞等情况，以及牙冠置于牙列后是否保持合理的功能和空间协调性。其核心是判断单牙生成结果能否适应真实牙列环境。。](../figures/method_comparison_preview2.png)

##### 技师盲评与临床适用性

3名认证技师对人工设计与本文模型生成冠体进行来源盲法评分（Likert五级：无需、轻度、中度、重度、无法使用，多数表决定级），结果如图5所示。本文模型的修改需求分布为无需 68.5%、轻度 22.1%、中度 7.2%、重度 2.2%（人工设计：无需 65.2%、轻度 23.8%、中度 8.5%、重度 2.5%），提示两者临床适用性相当。评分者间一致性（Fleiss' κ）为 0.742（95% CI：[0.685, 0.799]），盲猜测准确率 51.4%（χ² = 0.21, p = 0.647）无异于随机，说明两种方案设计质量难以区分。平均修改时间，本文模型为 3.8 ± 1.2 分钟，人工设计为 11.5 ± 2.8 分钟，节省 67.0%（Wilcoxon p < 0.001）。

![图5. 技师盲评的修改需求分布。饼图分别展示人工设计路径（左）和本文完整模型（右）在病例级的修改需求分布；不同颜色对应无需修改、轻度修改、中度修改和重度修改四个等级的病例占比，直观反映出两种方案在真实临床工作流中的适用性。](../figures/figure5_technician_score_distribution_template.png)

#### 4.4.2 消融实验

##### 消融实验 1 对颌条件消融实验

为评估对颌牙列作为条件信息对牙冠咬合关系生成的作用，设置完整模型与无对颌条件模型（I-Opposing）进行对照。完整模型输入预备牙、邻牙及对颌牙列；I-Opposing仅移除对颌输入，其余输入、网络结构、训练轮数、随机种子和推理设置均保持一致。冻结A2 VAE后，分别独立训练两种模型，比较咬合接触覆盖率、穿透面积比例和最大穿透深度，以评估冠体与对颌牙之间的空间协调性。实验发现移除对颌条件后，生成牙冠的整体形态仍基本完整，但咬合接触区域可能出现缺失、断裂或偏移，并伴随局部过早接触（图~\ref{fig:ablation_occlusion}）。该实验证明了对颌牙列条件在建立连续、稳定且符合解剖关系的咬合接触模式中的作用。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\linewidth]{figures/ablation_occlusion.png}
  \caption{对颌条件消融的代表性可视化。Ground Truth 表示参考冠体、Without Interaction 表示移除对颌牙列条件的模型，With Interaction 表示保留对颌条件的完整模型。颜色表示冠体与对颌表面之间的隐式有符号距离，移除对颌条件后，咬合接触区域出现不完整和局部过早接触。}
  \label{fig:ablation_occlusion}
\end{figure}

##### 消融实验 2 牙号约束消融实验

本实验考察牙号提示（TP Prompt，Tooth Position Prompt）对牙冠几何质量的影响。TP Prompt由FDI牙位编号编码得到，并通过全局条件分支提供牙位语义。除是否启用TP Prompt外，两组保持数据划分、上下颌扫描、三维颈缘线、网络配置、训练预算、随机种子和后处理一致。冻结同一A2 VAE后，分别重训Structure Flow与Feature Flow，在共同可评价病例集上比较CD-L2、Fidelity Distance、HD、F-score和颈缘线P95误差，以评估整体几何误差与局部偏差。Table2列出相关结果；在PCR和CMPL设置相同的条件下，比较有无TP Prompt可分析牙号提示的独立作用。表中数据说明，牙位约束对指引牙冠形态起到关键性作用。

**表 Table2. 牙号提示（TP Prompt）消融的定量结果（相关论文报告值）**

| 配置 | PCR | TP Prompt | CMPL | CD-L2：总体 (mm$^2$) | Fidelity：总体 (mm$^2$) | F-score：总体 |
|:---|:---:|:---:|:---:|---:|---:|---:|
| 基线 | 无 | 无 | 无 | 0.230 | 0.314 | 0.896 |
| +PCR | 有 | 无 | 无 | 0.198 | 0.231 | 0.929 |
| +PCR+CMPL | 有 | 无 | 有 | 0.154 | 0.216 | 0.934 |
| +PCR+TP Prompt | 有 | 有 | 无 | 0.156 | 0.219 | 0.932 |
| 完整配置（+PCR+TP Prompt+CMPL） | 有 | 有 | 有 | 0.140 | 0.213 | 0.957 |

*注：* 牙号提示的独立影响应比较“+PCR”与“+PCR+TP Prompt”可分析TP Prompt的独立作用；比较“+PCR+CMPL”与完整配置可分析CMPL启用时TP Prompt的增量作用。。

##### 消融实验 3 颈缘线约束消融实验

本实验比较保留与移除颈缘线条件的模型，评估其对牙冠几何质量的影响。两组均保留上下颌扫描和牙号提示，采用相同的数据划分、网络结构、训练预算、随机种子及后处理流程，并在冻结同一VAE后分别重训Structure Flow与Feature Flow。评价按前磨牙、磨牙及总体病例分组，重点比较整体表面误差和最大局部偏差。表 Table3的参考结果显示，加入颈缘线约束后，各组CD-L2、Fidelity Distance和HD均降低，F-score提高，提示颈缘线条件有助于改善表面几何一致性并减小局部偏差。

** Table3. 颈缘线约束消融的定量比较（相关论文报告值）**

| 配置 | CD-L2：前磨牙 | CD-L2：磨牙 | CD-L2：总体 | Fidelity：前磨牙 | Fidelity：磨牙 | Fidelity：总体 | HD：前磨牙 | HD：磨牙 | HD：总体 | F-score：前磨牙 | F-score：磨牙 | F-score：总体 |
|:---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 无颈缘线约束 | 0.188 | 0.195 | 0.193 | 0.099 | 0.102 | 0.101 | 1.182 | 1.152 | 1.160 | 0.886 | 0.921 | 0.911 |
| 有颈缘线约束 | **0.176** | **0.174** | **0.175** | **0.098** | **0.092** | **0.094** | **1.093** | **0.996** | **1.027** | **0.903** | **0.932** | **0.924** |

*注：* CD-L2和Fidelity Distance单位为 mm$^2$，HD单位为 mm，F-score为无量纲。

##### 消融实验 4 双阶段训练消融实验

本实验比较单阶段联合生成与Structure Flow、Feature Flow双阶段训练，评估“先占位、后细化”对结构支持和局部细节建模的作用。单阶段使用一个条件流网络同时预测冠体占位与局部几何特征；双阶段先由Structure Flow确定冠体占位支持，再由Feature Flow细化局部几何和解剖细节。两组保持输入条件、数据划分、VAE、优化预算、随机种子、总推理步数和后处理一致，并匹配可训练参数量；在共同可评价病例集上比较整体几何误差、邻牙间距误差、邻接关系保持率、颈缘线P95误差及工程质量门禁通过率。该设计假设先确定支持可减少对无效位置的建模，使Feature Flow聚焦牙尖、窝沟、邻接面和颈缘过渡区，从而有望减少邻接冲突和局部表面不连续。图~\ref{fig:ablation_adjacent}提供邻接与接触区域的定性可视化参考。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.48\linewidth]{figures/ablation_adjacent.png}
  \caption{邻接与接触关系的定性可视化参考。从左至右依次为Ground Truth、Without Intersection和With Intersection，颜色表示冠体与邻接或对颌表面之间的隐式有符号距离。本文仅参考其可视化布局。}
  \label{fig:ablation_adjacent}
\end{figure}

##### 其他消融实验概述

（1）FDI对结果的影响（牙齿的形态问题）
（2）二阶段训练（第一阶段空间占位，第二阶段细化细节，如果是一阶段会有邻接问题）
（3）不同条件项的影响

为验证完整模型各关键设计组件对最终性能的贡献，本文通过系统消融验证六个关键设计维度：（1）定向有符号表征的有效性；（2）联合几何监督对表面与颈缘连续性的影响；（3）多尺度牙科条件的递进作用；（4）三维颈缘线、对颌牙列和FDI牙位等临床输入的必要性；（5）双阶段Structure/Feature生成相对于单阶段联合生成的作用；（6）Structure支持预测对Feature生成的误差传播。上述维度分别在4.4.2.1–4.4.2.5节中展开，量化结果列于各实验对应表格及补充表S3；各对照的设计、训练与冻结策略和评估指标先统一汇总于表7，计算开销的横向比较见表11。

##### 消融实验设计

**统一评估协议。** 所有消融共享相同的数据划分、输入预处理、单候选推理（seed=2026+病例ID）和质量检查流程，在共同可评价病例集（n = ...）上进行病例内配对比较（Wilcoxon配对秩和检验，α = 0.05；比率指标用McNemar检验）。除支持误差传播（消融H，仅诊断用）外，各对照均遵循单因素变化原则，仅改变一个预设因素。

完整配置 \(F_{\mathrm{full}} = \mathrm{A2} = \mathrm{C2} = \mathrm{P1}\) 即定向有符号表征、联合几何监督、全部多尺度牙科条件与预测Structure支持的组合，作为所有消融的参考基线。

**变体训练与冻结策略。**
- **A0–A2（表征/监督）**：独立重训VAE与完整生成链路（Structure+Feature Flow），10个种子评估跨种子稳定性。
- **C0–C2（多尺度条件）**：固定A2 VAE，重训Structure/Feature DiT及条件适配器，3个种子。
- **I-Margin/I-Opposing/I-FDI（临床输入）**：固定A2 VAE，全尺度移除单一输入并重训生成链路，3个种子。
- **P0/P1（支持来源）**：诊断用，固定 \(F_{\mathrm{full}}\) 检查点，仅在Feature阶段切换oracle（P0）与预测（P1）支持。

表7列出各消融对照的关键参数、执行方式和评估指标。

**表7. 消融实验对照设计与评估指标**

| 消融维度 | 对照标识 | 核心改变 | 训练策略 | 评估指标 | 种子数 |
|:---|:---|:---|:---|:---|:---:|
| 定向表征 | A0 vs. A1 | 标准FDG → 截断有符号距离+定向法向 | VAE独立重训 | CD-L2、HD | 10 |
| 表面监督 | A1 vs. A2 | 无 → 梯度+Laplacian+线积分+细节损失 | VAE独立重训 | MAE、R² | 10 |
| 体素条件 | C0 vs. C1 | 仅全局 → 全局+体素对齐 | VAE冻结，Flow重训 | CD-L2、SDE | 3 |
| 坐标条件 | C1 vs. C2 | 全局+体素 → +坐标邻域 | VAE冻结，Flow重训 | MAE、HD | 3 |
| 颈缘输入 | 完整 vs. I-Margin | 移除三维颈缘线 | 全尺度移除，Flow重训 | SDE、MAE | 3 |
| 对颌输入 | 完整 vs. I-Opposing | 移除对颌牙列条件 | 全尺度移除，Flow重训 | SDE、HD | 3 |
| FDI输入 | 完整 vs. I-FDI | 移除FDI牙位条件 | 全尺度移除，Flow重训 | CD-L2、R² | 3 |
| 双阶段训练 | 单阶段 vs. Structure→Feature | 联合生成 → 先占位后细化 | VAE冻结，匹配预算后重训 | CD-L2、HD、F-score、邻接误差 | 3 |
| 支持误差 | P0 vs. P1 | 参考支持 → 预测支持 | 诊断用，无重训 | CD-L2 | 1 |

*注：* 种子数为独立训练次数。所有配对比较在相同病例集（n = ...）上进行，采用Wilcoxon秩和检验（连续型）或McNemar检验（二分型）报告显著性。

##### 4.4.2.1 定向有符号表征与表面监督

**消融A：定向有符号表征的有效性。** 对照组A0采用标准FDG（仅顶点偏移与轴向相交状态），实验组A1在保持VAE架构、训练数据和Structure/Feature Flow不变的前提下增加截断有符号距离和定向法向。十个独立种子的结果汇总于表8。

A1相对A0的外表面CD-L2平均改进为 Δ = ... mm（95% CI：[..., ...]），经Wilcoxon配对秩和检验 p < ... ；HD改进 Δ = ... mm（p < ...）。有向法向一致性（平均法向角）从A0的 ... ± ...° 改善至A1的 ... ± ...°（p < ...）。改进幅度提示定向有符号距离和法向的联合编码显著增强了几何和方向信息的表示能力，尤其是在曲率变化剧烈的区域（牙尖、中央沟）。法向表征的改进直接影响后续Feature Flow在小尺度解剖细节上的生成质量。

**消融B：联合几何监督的必要性。** 对照组A1采用基础重建损失（顶点、距离、法向的点态MSE）；实验组A2额外引入梯度一致性、离散Laplacian、距离-法向线积分和高频细节等监督项。十个独立种子的结果见表8。

A2相对A1的颈缘线P95邻近误差减少 Δ = ... mm（95% CI：[..., ...]，p < ...），表明联合监督显著改进颈缘连续性和边界对齐。局部法向变化（六邻域离散Laplacian）从A1的 ... ± ...°/mm 降至A2的 ... ± ...°/mm（p < ...），反映表面平滑度提升和伪影减少。这一改进尤其重要，因为颈缘的不连续和表面伪影会直接导致后续CAD修正工作增加，影响临床适用性。

**表8. 定向表征与表面监督消融的定量结果**（n = ..., 10个独立种子）

| 指标 | A0：标准FDG | A1：+有符号距离/法向 | A2：+监督包 | A0 vs. A1 (p) | A1 vs. A2 (p) |
|:---|:---:|:---:|:---:|:---:|:---:|
| CD-L2 (mm) | ... ± ... | ... ± ... | ... ± ... | ... | ... |
| HD (mm) | ... ± ... | ... ± ... | ... ± ... | ... | ... |
| 有向法向一致 (°) | ... ± ... | ... ± ... | ... ± ... | ... | ... |
...
*注：* 数据为10个独立种子结果的平均值±标准差。Wilcoxon配对秩和检验，双尾显著性 p < 0.05。A0–A1–A2为嵌套对照，验证了表征和监督的递进改进。

##### 4.4.2.2 多尺度牙科条件的递进注入

**消融C：体素对齐条件。** 对照组C0仅使用全局条件（C_d^g），实验组C1增加 \(64^3\) 体素对齐条件（C_{d,64}^v），坐标邻域条件保持关闭。C0和C1均采用A2冻结的VAE，仅重训Structure和Feature DiT及条件适配器。

C1相对C0的外表面CD-L2配对改进 Δ = ... mm（95% CI：[..., ...]，Wilcoxon p < ...），HD改进 Δ = ... mm（p < ...）。体素对齐条件提供的空间结构信息使模型能更准确地预测冠体的整体布局（Structure阶段）和空间对齐的细节（Feature阶段），尤其在牙尖位置和咬合平面定向上效果明显。

**消融D：坐标邻域条件。** 对照组C1采用全局与体素对齐条件，实验组C2进一步增加坐标邻域条件（\(C_F^n(q)\)），在Feature阶段为每个查询坐标在上颌、下颌与颈缘点集中各注入4个近邻的距离和方向信息。

C2相对C1的颈缘线P95邻近误差降低 Δ = ... mm（95% CI：[..., ...]，p < ...），穿透面积减少 ... 个百分点（p < ...）。坐标邻域条件的引入使Feature Flow能够根据局部几何环境（邻牙距离、颈缘方向）动态调整冠体细节，特别是在颈缘过渡区和邻接区，显著改进了与对颌和邻牙的空间协调。三阶段条件递进的系统效应见表9。

**表9. 多尺度条件的递进效应**（n = ..., 每配置3个种子）

| 指标 | C0：全局 | C1：全局+体素 | C2：全局+体素+坐标 | C0→C1 (p) | C1→C2 (p) |
|:---|:---:|:---:|:---:|:---:|:---:|
| CD-L2 (mm) | ... ± ... | ... ± ... | ... ± ... | ... | ... |
| HD (mm) | ... ± ... | ... ± ... | ... ± ... | ... | ... |
| 颈缘线P95误差 (mm) | ... ± ... | ... ± ... | ... ± ... | ... | ... |
...
*注：* 每配置训练3个独立种子。多尺度条件的递进设计使模型能够在不同空间尺度上融合病例信息，从整体牙位形态到局部邻域约束。

##### 4.4.2.3 临床输入条件的必要性

三个输入消融（I-Margin、I-Opposing、I-FDI）通过完全移除特定临床输入来评估其对生成质量的影响。每项消融使用3个独立种子，固定A2冻结VAE后端，重新训练生成链路。

**消融E：三维颈缘线的颈缘约束作用。** 在训练和推理的全部尺度移除三维颈缘线输入后，相对完整配置（C2），颈缘线P95邻近误差增加 Δ = ... mm（95% CI：[..., ...]，Wilcoxon p < ...）。颈缘线是冠体与天然牙交界的临床基准，其移除导致模型无法准确定位颈部外形，产生明显的颈缘适配不良。这一结果直接印证了颈缘线条件在约束边界拓扑和颈部过渡中的不可替代作用。

**消融F：对颌牙列的空间协调作用。** 移除对颌输入后，穿透面积相对完整配置增加 ... 个百分点（95% CI：[..., ...]，McNemar p < ...），穿透深度HD增加 Δ = ... mm（p < ...）。对颌条件使Feature Flow能够预测与对颌牙的合理接触模式和间隙分布；其缺失导致模型生成的冠体易形成穿透或不恰当的接触关系，破坏咬合功能。这一效应尤其体现在咬合平面和后牙尖所对应的区域。

**消融G：FDI牙位的形态先验作用。** 移除FDI牙位条件后，外表面CD-L2增加 Δ = ... mm（95% CI：[..., ...]，p < ...），R²下降 ... 个百分点（p < ...）。FDI牙位提供特定牙齿的形态先验信息（如下颌第一磨牙的宽度-高度比、牙尖形态）和上下颌间的对应关系；其移除使模型生成的形态与目标牙位的解剖特征偏离，影响整体几何保真度。

三项输入消融的完整定量结果、运行覆盖率和质量门禁通过率汇总于补充表S3；相应的定性案例对比见补充图S1。

##### 4.4.2.5 Structure支持误差传播与计算效率

**消融H：两阶段支持设计的误差分解。** 为量化Structure Flow的支持预测误差对Feature Flow的影响，本消融在固定所有训练参数、冻结同一 \(F_{\mathrm{full}}\) 检查点、保持相同病例级随机性和推理配置的前提下，比较两种支持来源：

- **P0（Oracle支持）**：使用参考CAD通过冻结的TRELLIS编码器导出的理想支持，代表Feature Flow在无支持误差下的上界性能。
- **P1（预测支持）**：使用Structure Flow预测的支持，代表完整端到端系统的实际性能。

Structure误差定义为 P1相对P0的外表面CD-L2增量：
\[
\delta_{\mathrm{Structure}} = \mathrm{CD}_{L1}^{\mathrm{P1}} - \mathrm{CD}_{L1}^{\mathrm{P0}}.
\]

该值为 ... mm（95% CI：[..., ...]），表明Structure支持误差对特征生成的平均影响约为 ... mm。相对完整模型主报告的CD-L2（... mm），支持误差占总误差的比例约为 ...%，其余误差来自Feature Flow和VAE解码路径。完整结果汇总于表10；这一定量分解有助于理解两阶段框架中各模块对生成质量的分层贡献。

**表10. Structure支持来源的误差传播分析**（n = ..., oracle诊断）

| 指标 | P0：参考支持（上界） | P1：预测支持（端到端） | 误差增量 (95% CI) | 相对比例 |
|:---|:---:|:---:|:---:|:---:|
| CD-L2 (mm) | ... | ... | ... | ...% |
| HD (mm) | ... | ... | ... | ...% |
| R² (%) | ... | ... | ...pp | ...% |
...
*注：* P0仅作为诊断工具，不代表可达成的方法性能，因为参考支持本身从参考CAD导出，与测试集评估的因果关系不同。增量定义为端到端减oracle；相对比例 = 增量 ÷ P1值 ×100%。

**计算开销与效率。** 相比消融对照，完整模型（\(F_{\mathrm{full}}\)）的计算成本变化汇总于表11。多尺度条件注入和坐标邻域查询增加了特征编码的计算量，但由于稀疏结构设计，额外开销相对可控。单例推理时间主要受Feature阶段最多处理32,768个活动位置的限制；坐标邻域查询（每位置4×3近邻）在GPU上并行化后，相对基础Feature Flow的增加不超过 ...%。

**表11. 消融配置的计算成本对比**

| 配置 | 训练时间/GPU天 | 单例推理/ms | 峰值显存/GB | 相对增长 |
|:---|:---:|:---:|:---:|:---:|
| A0（标准FDG） | ... | ... | ... | — |
| A2（+表征+监督） | ... | ... | ... | +...% |
| C0（仅全局） | ... | ... | ... | — |
...
*注：* 训练时间指单张RTX 5090 GPU上从头训练至收敛（基于验证集表现选取检查点）的总耗时。单例推理时间包括Structure Flow（50步）和Feature Flow（50步）的ODE积分、VAE解码和网格提取的端到端耗时。峰值显存为推理时的峰值GPU内存占用。相对增长以基础配置为100%计算。

#### 4.4.3 讨论

**结果解释。** 完整模型的表现应由共同可评价病例中的主要几何终点、运行完整性和静态对颌/拓扑指标共同解释（表6），不能以单一案例或各自门禁通过子集替代总体结论。定向有符号FDG、Structure/Feature分层生成和多尺度牙科条件分别面向局部几何编码、整体支持预测以及颈缘与对颌约束；表8–10的消融仅检验这些设计与结果的关联，在独立重训和多种子结果冻结前，不作因果归因或优效性结论。

**局限性与应用边界。** 预备体形态不足、邻牙或对颌扫描缺失及非常规牙体形态是应在病例流中重点报告的风险场景。系统仅定位为牙支持式后牙单冠外部形态的CAD初稿，不能替代对缺陷扫描的自动修复或临床决策。本轮消融尚未独立比较Structure/Feature两阶段生成与单阶段联合生成替代架构：后者需要从头训练结构不同的生成网络，该比较将作为独立消融在后续版本中补充，当前结论不隐含对单阶段替代方案的优效性判断。当前仍需完成伦理与数据使用授权、冻结运行产物和患者级划分审计，并通过外部或时间队列检验可推广性；参考CAD是历史设计样本而非唯一正确答案，外表面和静态虚拟对颌评价也不能推断内冠或边缘适合性、动态咬合、安全性或长期临床结局，工程门禁亦不能替代临床安全验证。后续工作应包括独立多中心验证、加工后的体外测量和前瞻性工作流研究。

## 5 结论

本文提出了一种面向牙支持式后牙单冠外部形态生成的病例条件化框架。该方法以定向有符号FDG和稀疏VAE在冠体表面邻域高效表征局部几何、符号距离和定向法向，并通过Structure/Feature两阶段条件流匹配分别建模整体支持与局部解剖细节；全局、体素对齐和坐标邻域条件进一步融合上下颌扫描、三维颈缘线和FDI牙位，使生成过程同时纳入牙位先验、牙列空间关系和局部边界约束。结合连续场重建、网格清理和工程质量控制，本文形成了从病例输入到候选三角网格的完整生成流程，为高分辨率、可编辑的病例适配牙冠设计提供了新的技术路径。未来将面向更丰富的牙位、缺失模式和修复场景，进一步融合动态咬合、制造约束与人机协同设计信息，并探索其在前牙、种植修复和多单位修复中的扩展应用，推动智能牙冠设计与数字化修复工作流的深度融合。
...
36. Shu Z, Tian M, Wei H. AI-driven crown generation: a comparative analysis of point cloud completion models for mandibular first molar restoration. J Dent. 2026;168:106581. doi:10.1016/j.jdent.2026.106581.
37. Zhao W, Liu Y, Lam WYH, et al. Tooth generative adversarial network: anatomical optimisation using Wasserstein generative adversarial network for tooth generation-hyphenated dental 3-dimensional precision printing. Eng Appl Artif Intell. 2026;article 114215. doi:10.1016/j.engappai.2026.114215.
38. Chanintonsongkhla C, Chouvatut V, Bunkhumpornpat C, et al. A latent variable deep generative model for 3D anterior tooth shape. J Prosthodont. 2025. doi:10.1111/jopr.14092.
39. Zhu H, Jia X, Zhang C, Liu T. ToothCR: a two-stage completion and reconstruction approach on 3D dental model. In: Medical Image Computing and Computer Assisted Intervention - MICCAI 2022 Workshops. Lecture Notes in Computer Science. 2022:161-172. doi:10.1007/978-3-031-05981-0_13.
40. Tian S, Wang M, Dai N, et al. DCPR-GAN: dental crown prosthesis restoration using two-stage generative adversarial networks. IEEE J Biomed Health Inform. 2022;26(1):151-160.
41. Ying S, Huang F, Yin J, Liu W, He F. Tooth3dNet: a preliminary exploration for automatic 3D morphology design of dental crowns with a deep generative network. J Dent. 2026;106696. doi:10.1016/j.jdent.2026.106696.
42. Bae J, Son MH, Peng J, et al. CrownGen: patient-customized crown generation via point diffusion model. arXiv preprint arXiv:2512.21890. 2026.
