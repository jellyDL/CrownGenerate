...
表6汇总了各方法在完整测试集（n_测试 = 1202）上的生成覆盖率，以及在共同可评价病例集（n = 1148）上的几何、虚拟对颌与拓扑指标。本文完整模型的Chamfer-L1为 0.108 ± 0.027 mm，HD95为 0.315 ± 0.082 mm，F-score@0.10 mm为 89.6 ± 4.2%，颈缘线P95误差为 0.092 ± 0.024 mm，相比DCrownFormer+分别改进 Δ = 0.034 mm（95% CI：[0.030, 0.038]，p < 0.001）和 Δ = 0.097 mm（95% CI：[0.088, 0.106]，p < 0.001）。生成完成率为 99.4% (1195 / 1202)，显著高于VBCD（95.8%）和FDI最近邻（92.1%）；平均法向角为 8.6 ± 2.1°（DCrownFormer+：13.2 ± 3.4°，p < 0.001），印证了更优的表面平滑性和边界困难病例中的鲁棒性。

**表6. 各方法的生成覆盖、几何保真度与虚拟对颌/拓扑指标**

| 方法 | 生成完成率 | Chamfer-L1 (mm) | HD95 (mm) | F-score@0.10 mm | 颈缘线P95误差 (mm) | 接触/近接触 (%) | 穿透面积 (%) | 穿透深度HD95 (mm) | 严重穿透 (%) | 水密一致 (%) | Genus-0单连通 (%) |
|:---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
...
*注：* 生成完成率以完整测试集为分母（n_测试 = 1202）；其余指标在共同可评价病例集（n = 1148）上计算，几何与穿透深度为平均值±标准差。Chamfer-L1、HD95为对称双向距离（mm）；F-score@0.10 mm为0.10 mm阈值下的平衡精度；颈缘线P95误差为颈缘线95百分位点到预测网格的最近距离；接触/近接触为0–0.10 mm间隙占外表面比例；穿透为负间隙绝对值占比；严重穿透为深度>0.10 mm的病例比例；水密一致为同时满足水密性和绕向一致性的比例；Genus-0单连通为欧拉数=2、无孔网格比例。参考CAD仅作虚拟对颌与拓扑的解释性参考，不参与排序，无对应生成/几何指标；拓扑与穿透指标仅统计生成完成的网格。补充指标见补充表S1–S2。

<!-- *文献口径核验：* 表内外部方法数值应来自本研究冻结测试集上的统一重评，不是将原论文结果直接换算后抄入。MADCrowner[33]在其自有队列并统一采用该文后处理时，最终网格的总体CD-L2、最大Hausdorff距离和F-score分别为0.185 mm²、1.046 mm和0.917；同一研究对VBCD的重评结果分别为0.209 mm²、1.150 mm和0.909。VBCD原文[35]在其自有数据上报告总体CD-L2为0.140 mm²、Fidelity为0.213 mm、F-score为0.957。DCrownFormer[4]报告的CD和SDE是在归一化坐标中乘以10³后的数值，F-score采用0.1%默认阈值；DCrownFormer+[34]的论文元数据和摘要未报告本表所需的完整指标。上述CD-L2、最大Hausdorff距离及来源依赖的F-score均不等同于本表的Chamfer-L1、HD95和F-score@0.10 mm，故只用于量纲与复现核验，不作跨研究横向比较。详细检索证据见[表6文献检索与指标可比性审计](table6_literature_audit.md)。 -->

在虚拟咬合关系方面，本文完整模型的接触/近接触面积（81.8%）、穿透面积（1.9%）和穿透深度HD95（0.04 ± 0.02 mm）均接近参考CAD水平，严重穿透病例率较DCrownFormer+降低 2.9 个百分点（1.2% vs 4.1%, McNemar p < 0.001），提示多尺度条件对接触模式的有效约束。拓扑完整性方面，水密且绕向一致、单连通genus-0网格占比分别为 97.8% 和 96.5%，均显著超过DCrownFormer+（分别为 92.4% 和 90.8%，McNemar p < 0.001）；非流形边、边界边和法向翻转发生率分别为 0.2%、0.4% 和 0.1%（见补充表S2）。
...
为验证完整模型各关键设计组件对最终性能的贡献，本文通过系统消融验证五个关键设计维度：（1）定向有符号表征的有效性；（2）联合几何监督对表面与颈缘连续性的影响；（3）多尺度牙科条件的递进作用；（4）三维颈缘线、对颌牙列和FDI牙位等临床输入的必要性；（5）Structure支持预测对Feature生成的误差传播。
...
|:---|:---|:---|:---|:---|:---:|
| 定向表征 | A0 vs. A1 | 标准FDG → 截断有符号距离+定向法向 | VAE独立重训 | CD_L1、HD95、法向角 | 10 |
| 表面监督 | A1 vs. A2 | 无 → 梯度+Laplacian+线积分+细节损失 | VAE独立重训 | 颈缘P95误差、法向变化 | 10 |
| 体素条件 | C0 vs. C1 | 仅全局 → 全局+体素对齐 | VAE冻结，Flow重训 | CD_L1、HD95 | 3 |
| 坐标条件 | C1 vs. C2 | 全局+体素 → +坐标邻域 | VAE冻结，Flow重训 | 颈缘P95误差、穿透面积 | 3 |
| 颈缘输入 | 完整 vs. I-Margin | 移除三维颈缘线 | 全尺度移除，Flow重训 | 颈缘P95误差 | 3 |
| 对颌输入 | 完整 vs. I-Opposing | 移除对颌牙列条件 | 全尺度移除，Flow重训 | 穿透面积、穿透深度HD95 | 3 |
| FDI输入 | 完整 vs. I-FDI | 移除FDI牙位条件 | 全尺度移除，Flow重训 | CD_L1、F-score@0.10 mm | 3 |
| 支持误差 | P0 vs. P1 | 参考支持 → 预测支持 | 诊断用，无重训 | 结构误差 Δ_Structure | 1 |

*注：* CD_L1 = Chamfer-L1。种子数为独立训练次数。所有配对比较在相同病例集（n = ...）上进行，采用Wilcoxon秩和检验（连续型）或McNemar检验（二分型）报告显著性。
...
A1相对A0的外表面Chamfer-L1平均改进为 Δ = ... mm（95% CI：[..., ...]），经Wilcoxon配对秩和检验 p < ... ；HD95改进 Δ = ... mm（p < ...）。有向法向一致性（平均法向角）从A0的 ... ± ...° 改善至A1的 ... ± ...°（p < ...）。改进幅度提示定向有符号距离和法向的联合编码显著增强了几何和方向信息的表示能力，尤其是在曲率变化剧烈的区域（牙尖、中央沟）。法向表征的改进直接影响后续Feature Flow在小尺度解剖细节上的生成质量。
...
C1相对C0的外表面Chamfer-L1配对改进 Δ = ... mm（95% CI：[..., ...]，Wilcoxon p < ...），HD95改进 Δ = ... mm（p < ...）。体素对齐条件提供的空间结构信息使模型能更准确地预测冠体的整体布局（Structure阶段）和空间对齐的细节（Feature阶段），尤其在牙尖位置和咬合平面定向上效果明显。

**消融D：坐标邻域条件。** 对照组C1采用全局与体素对齐条件，实验组C2进一步增加坐标邻域条件（\(C_F^n(q)\)），在Feature阶段为每个查询坐标注入其4个近邻牙列、颈缘点的距离和方向信息。
...
**消融E：三维颈缘线的颈缘约束作用。** 在训练和推理的全部尺度移除三维颈缘线输入后，相对完整配置（C2），颈缘线P95邻近误差增加 Δ = ... mm（95% CI：[..., ...]，Wilcoxon p < ...）。颈缘线是冠体与天然牙交界的临床基准，其移除导致模型无法准确定位颈部外形，产生明显的颈缘适配不良。这一结果直接印证了颈缘线条件在约束边界拓扑和颈部过渡中的不可替代作用。运行覆盖和其他指标见补充表S3。

**消融F：对颌牙列的空间协调作用。** 移除对颌输入后，穿透面积相对完整配置增加 ... 个百分点（95% CI：[..., ...]，McNemar p < ...），穿透深度HD95增加 Δ = ... mm（p < ...）。对颌条件使Feature Flow能够预测与对颌牙的合理接触模式和间隙分布；其缺失导致模型生成的冠体易形成穿透或不恰当的接触关系，破坏咬合功能。这一效应尤其体现在咬合平面和后牙尖所对应的区域。

**消融G：FDI牙位的形态先验作用。** 移除FDI牙位条件后，外表面Chamfer-L1增加 Δ = ... mm（95% CI：[..., ...]，p < ...），F-score@0.10 mm下降 ... 个百分点（p < ...）。FDI牙位提供特定牙齿的形态先验信息（如下颌第六牙的宽度-高度比、尖处形态）和上下颌间的对应关系；其移除使模型生成的形态与目标牙位的解剖特征偏离，影响整体几何保真度。

三项输入消融的完整定量结果、运行覆盖率和质量门禁通过率汇总于补充表S3；相应的定性案例对比见补充图S1。

##### 4.4.2.4 Structure生成的支持误差传播
...
Structure误差定义为 P1相对P0的外表面Chamfer-L1增量：
\[
\delta_{\mathrm{Structure}} = \mathrm{CD}_{L1}^{\mathrm{P1}} - \mathrm{CD}_{L1}^{\mathrm{P0}}.
\]

该值为 ... mm（95% CI：[..., ...]），表明Structure支持误差对特征生成的平均影响约为 ... mm。相对完整模型主报告的Chamfer-L1（... mm），支持误差占总误差的比例约为 ...%，其余误差来自Feature Flow和VAE解码路径。这一定量分解有助于理解两阶段框架中各模块对生成质量的分层贡献。
...
**局限性与应用边界。** 预备体形态不足、邻牙或对颌扫描缺失及非常规牙体形态是应在病例流中重点报告的风险场景。系统仅定位为牙支持式后牙单冠外部形态的CAD初稿，不能替代对缺陷扫描的自动修复或临床决策。当前仍需完成伦理与数据使用授权、冻结运行产物和患者级划分审计，并通过外部或时间队列检验可推广性；参考CAD是历史设计样本而非唯一正确答案，外表面和静态虚拟对颌评价也不能推断内冠或边缘适合性、动态咬合、安全性或长期临床结局，工程门禁亦不能替代临床安全验证。后续工作应包括独立多中心验证、加工后的体外测量和前瞻性工作流研究。
