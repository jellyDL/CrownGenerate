...
\begin{figure*}[h]  % [t] 表示尽量放在页面顶部
    \centering % 图片居中
    \includegraphics[width=0.9\textwidth]{./figures/feature_condition_detail.pdf}
...
%% 4.2.1
\subsubsection{Geometric and Surface Metrics}\label{subsec4_2_1}

\hspace{2em}The reference CAD model and predicted closed mesh were topologically cut along the same ordered cervical margin curve. The larger patch containing the cusps and occlusal surface was defined as the clinical outer surface; the other patch was treated as the base cap. Geometric metrics were computed between outer surfaces, each sampled at 50,000 triangle-area-weighted points.

\hspace{2em}The primary endpoint was the symmetric Chamfer-L1 distance:
\[
...
\hspace{2em}Secondary metrics included HD95, Chamfer-L2, bidirectional P95 distance, normal consistency, mean/P95 normal angular error, F-scores at 0.05, 0.10, and 0.20~mm, area ratio, local normal variation, P95 dihedral angle, and integrated absolute curvature. Cervical margin proximity error was defined as the mean and P95 nearest distances from original margin points to the predicted mesh.

%% 4.2.2 
\subsubsection{Static Antagonist and Topological Metrics}\label{subsec4_2_2}

\hspace{2em}Signed distance \(d_o\) from the crown outer surface to the antagonist mesh was positive for clearance and negative for penetration. Thresholds were 0--0.02~mm for contact, 0.02--0.10~mm for near contact, and 0.10--0.20~mm for exploratory clearance; \(-0.02\)--0~mm for minor penetration, below \(-0.02\)~mm for substantial penetration, and below \(-0.10\)~mm for severe penetration. We reported contact/near-contact and penetration area fractions, mean/HD95/maximum penetration depth, and severe-penetration incidence.

\hspace{2em}Topological metrics included watertightness, orientation consistency, boundary- and non-manifold-edge ratios, adjacent-face normal-flip rate, Euler characteristic, genus, number of connected components, largest-component area fraction, and a single-component genus-0 validity gate.
