...
![图4. 典型病例的定性比较。按列展示参考设计CAD、FDI最近邻、MADCrowner、VBCD、DCrownFormer+和本文完整模型，按行展示整体冠体（上）、咬合沟窝细节（中）及颈缘区域（下）。误差图为各预测网格相对于参考CAD外表面的对称Chamfer距离，使用统一的色标范围（蓝：<0.05 mm，绿：0.05–0.10 mm，黄：0.10–0.20 mm，红：>0.20 mm）。](../figures/figure4_method_comparison_illustrative_preview.png)
...
##### 消融实验 1 对颌条件对牙冠咬合关系的独立消融

为评估对颌牙列作为条件信息对牙冠咬合关系生成的作用，设置完整模型与无对颌条件模型（I-Opposing）进行对照。完整模型同时输入预备牙、邻牙及对颌牙列，以提供冠体形态、邻接关系和上下颌空间关系；I-Opposing 移除对颌牙列条件，仅保留其他输入，并保持网络结构、训练轮数、随机种子和推理设置一致。在冻结 A2 VAE 的条件下，分别独立训练两种模型，并比较其咬合接触覆盖率、穿透面积比例和最大穿透深度。该消融重点考察对颌条件是否能够引导模型学习冠体与对颌牙之间的空间对应关系。预期移除对颌条件后，生成牙冠的整体形态仍基本完整，但咬合接触区域可能出现缺失、断裂或位置偏移，并伴随局部过早接触（见图~\ref{fig:ablation_occlusion}）。该实验证明了对颌牙列条件在建立连续、稳定且符合解剖关系的咬合接触模式中的作用。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\linewidth]{figures/ablation_occlusion.png}
  \caption{对颌条件消融的代表性可视化。Ground Truth 表示参考冠体，Without Interaction 表示移除对颌牙列条件的模型，With Interaction 表示保留对颌条件的完整模型。颜色表示冠体与对颌表面之间的隐式有向距离；移除对颌条件后，咬合接触区域出现不完整和局部过早接触。}
  \label{fig:ablation_occlusion}
\end{figure}

##### 消融实验 2 牙号约束消融实验

本实验考察牙号提示（TP Prompt，Tooth Position Prompt）对牙冠几何质量的影响。TP Prompt由FDI牙位编号编码得到，并在全局条件分支注入牙位语义。除是否启用TP Prompt外，两组保持数据划分、上下颌扫描、三维颈缘线、网络参数、A2 VAE、训练预算、随机种子和后处理一致；A2 VAE冻结后分别重训Structure/Feature Flow，并在共同可评价病例集上比较 CD-L2、Fidelity Distance、HD、F-score 和颈缘线P95误差。启用 TP Prompt 后，重点观察整体几何误差、最坏局部偏差和牙位相关形态保真度是否同步变化；若仅部分指标改善，应分别报告其差异及置信区间，不将单一指标变化概括为总体性能提升。具体数值、变化幅度和统计检验结果统一列于表 Table2。
...
*注：* 牙号提示的独立影响应比较“+PCR”与“+PCR+TP Prompt”两行；完整配置用于显示与 CMPL 联合后的结果。该表不包含 HD 和颈缘线 P95，因为相关论文未报告这两项指标；若用于本文正式结果，仍需按本文统一测试集和评价协议重新计算。

##### 消融实验 3 颈缘线约束消融实验

本实验比较保留与移除颈缘线条件的模型，以评估颈缘线约束对牙冠几何质量的影响。两组均保留上下颌扫描和牙号提示，并采用相同的数据划分、网络结构、训练预算、随机种子和后处理流程；冻结同一VAE后，分别重训Structure Flow与Feature Flow。评价重点为整体表面误差和最坏局部偏差，并按前磨牙、磨牙及总体病例分层比较。表 Table3 所列参考结果显示，加入颈缘线约束后，CD-L2、Fidelity Distance和HD均降低，F-score提高，且前磨牙与磨牙呈一致变化方向。这些差异提示颈缘线条件有助于改善表面几何一致性并控制局部最大偏差。

**表 Table3. 颈缘线约束消融的定量比较（相关论文报告值）**
...
本实验评估Structure Flow与Feature Flow分阶段训练对结构支持和局部细节建模的作用：单阶段对照使用一个条件流网络同时预测冠体占位与局部几何特征，双阶段配置则先由Structure Flow生成冠体占位支持，再由Feature Flow在该支持上细化局部几何和解剖细节。两种配置保持输入条件、数据划分、VAE、优化预算、随机种子、推理步数和后处理流程一致，并将可训练参数量控制在预设匹配范围内；在共同可评价病例集上比较整体几何误差、邻牙间距误差、邻接关系保持、颈缘线P95误差和工程质量门禁通过率。该设计假设显式支持依赖能够减少无效位置建模，使Feature Flow集中处理尖峰、沟裂、邻接面和颈缘过渡等局部区域，从而降低邻接冲突与局部断裂；图\ref{fig:ablation_adjacent}作为邻接/接触区域的定性可视化参考。

![图：双阶段训练消融的邻接关系定性参考](figures/ablation_adjacent.png)

*图：* 双阶段训练消融的邻接/接触关系定性参考图。各行示例按 Ground Truth、Without Intersection 和 With Intersection 排列，颜色表示冠体与邻接或对颌表面之间的隐式距离。本文仅借用其可视化组织方式
